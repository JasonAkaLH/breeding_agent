...
- PostgreSQL State Platform 子 PRD 已按 Phase 落入 `docs/prd/backend/postgresql-state-platform/`：新增 README、Phase 0 Driver/Contract、Phase 1 PostgreSQL Schema/Write Queue Kernel、Phase 2 Command Handlers/ReadStore/StateService、Phase 3 Runtime Integration/Fail-closed/Observability、Phase 4 SQLite->PostgreSQL Migration/Cutover 及对应 test spec，并同步 PRD 总索引与后端总览；仍不改运行时代码、不配置远端库、不执行迁移。
