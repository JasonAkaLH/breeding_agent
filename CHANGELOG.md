...
- 新增 PRD01 Rust quality gate 执行面：`.github/workflows/rust-quality.yml` 覆盖 fmt / clippy / cargo test / nextest / audit / deny / llvm-cov、Ubuntu 22.04 x86_64 Skill Runtime PyO3 wheel smoke、artifact provenance self-check 与 bounded fuzz smoke；`scripts/run_rust_quality_gates.py` 提供本地 gate matrix / partial runner；`native/fuzz/` 新增 Skill Runtime、MCP protocol、artifact path、audit sanitizer 与 readonly data-access fuzz harness，并补充静态回归测试。真实 CI 运行结果、coverage 阈值报告、长时 fuzz、真实 SBOM / provenance 产物仍待后续 release gate 证据。
