...
3. OCR 服务地址和 token 从本地 `config.yaml` 的 `ocr_mcp.base_url` / `ocr_mcp.auth_token` 启动注入，或从部署环境变量 `OCR_MCP_BASE_URL` / `OCR_MCP_AUTH_TOKEN` 注入；不要把真实连接信息写入 tracked 文件。
...
- 长任务排查时可临时把 `scripts/run_ocr.py` 顶部 `DEFAULT_DEBUG_PROGRESS` 改为 `True`；脚本只向 stderr 输出脱敏进度，stdout 仍保持单个 JSON object。
...
- 不要在回答中暴露 `OCR_MCP_AUTH_TOKEN` 或完整鉴权头。
- 更换远端服务时只改本地 `config.yaml` 或部署环境变量，不要把真实地址、token 或鉴权头提交到仓库。
