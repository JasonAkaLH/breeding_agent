...
class SQLQueryIntentRouteTest(unittest.TestCase):
    ...
    def test_genotype_intent_keyword_still_uses_configured_llm_semantic_layer(self) -> None:
        prompts: list[str] = []
    ...
    def test_approval_intent_keyword_still_uses_configured_llm_semantic_layer(self) -> None:
        prompts: list[str] = []

        async def semantic_router(prompt: str) -> str:
            prompts.append(prompt)
            return json.dumps({"intent": "database", "route_id": "approval_variety_db", "cross_crop_allowed": True}, ensure_ascii=False)
    ...
    def test_generic_variety_query_without_llm_requires_route_clarification(self) -> None:
        ...
        self.assertEqual(
            result.interrupt.required_fields["route_id"],
            {"options": ["approval_variety_db", "genotype_db"]},
        )
        self.assertEqual(
            result.interrupt.required_fields["_sql_query_resolution"]["presentation"],
            "natural_language",
        )
    ...
    def test_valid_llm_semantic_router_preserves_entities(self) -> None:
                    ...
                    "entities": [
                        {
                            ...
                            "secondary_fields": ["breeder", "transgenic_owner"],
                            "confidence": 0.8,
                        },
                        {"text": "河南", "entity_type": "region", "field_intent": "unknown"},
                    ],
                },
                ensure_ascii=False,
            )
        ...
        capability = SQLQueryIntentRouteCapability(semantic_text_generator=semantic_router)
        request = make_request("skill.sql_query", input_payload={"user_question": "隆平高科有哪些"})
        ...
        self.assertEqual(result.output_payload["route_id"], "approval_variety_db")
        self.assertEqual(
            result.output_payload["entities"],
            [
                {
                    "text": "隆平高科",
