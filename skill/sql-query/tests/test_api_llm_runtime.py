...
class SQLQueryLLMRuntimeAPITest(SQLQueryAPITestCase):
    ...
    async def test_sqlquery_llm_adapter_inherits_selected_model_and_thinking_options(self) -> None:
        ...
        prompt_names = [_sqlquery_prompt_name(call["prompt"]) for call in main_calls]
        self.assertEqual(prompt_names, ["sql_generate", "result_filtering"])
        self.assertEqual([call["thinking"] for call in main_calls], [True, True])
        self.assertEqual([call["reasoning_effort"] for call in main_calls], ["max", "max"])
        self.assertEqual([call["model_edition"] for call in main_calls], ["pro", "pro"])
