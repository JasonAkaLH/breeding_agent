...
> 适用范围：本模板用于 `skill.sql_query` 内部 SQL generation 阶段的提示词输入拼装，不覆盖路由识别、SQL 执行或 `result_filtering` 候选结果筛选阶段。
>
> 参考来源：
> - `docs/prd/backend/00-主代理框架PRD.md`
> - `docs/prd/backend/06-SQLQuery-MVP设计.md`
> - `docs/prd/backend/07-SQLQuery-LLM增强与真实库验证.md`
> - `skill/sql-query/configs/routing_rules.yaml`
> - `skill/sql-query/configs/schema_metadata.yaml`
> - `skill/sql-query/configs/sql_guard_rules.yaml`
...
注入来源：`skill/sql-query/configs/routing_rules.yaml`
...
注入来源：`skill/sql-query/configs/schema_metadata.yaml`
...
注入来源：
- `skill/sql-query/configs/sql_guard_rules.yaml`
- `skill/sql-query/configs/routing_rules.yaml` 中的 `sql_policy_profile`
