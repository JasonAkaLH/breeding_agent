...
- **状态**：2026-05-13 已完成 native SQLQuery capability 残留清理；当前公开入口仅为 `skill.sql_query`，handler key 为 `skill.sql_query.platform_handler`。
...
- **范围**：SQLQuery 领域服务拆分、`skill.sql_query` platform-service handler、API runtime 装配、Planner / Router / Replanner 去 SQLQuery 特判、前端 SQLQuery 展示与旧 artifact/event 展示 fallback；旧 capability alias 不再保留为请求入口。
...
4. **迁移最终必须删除 native capability。**
   过渡期可以先让 native SQLQuery executor 和 `skill.sql_query` 共用同一个 domain engine 并做输出对比；完成验收后必须移除 native capability、旧 provider/executor/replanner 和旧 request alias。
...
→ API 边界拒绝旧 sql_query / sql_query.query 请求 alias
...
- `src/api/runtime.py`
  - 已接入 `SkillExecutor`；
  - 已在 `SkillServiceRegistry` 中预留 `mysql_readonly`、`llm.sql_query`、`artifact_writer`、`progress_events` 等 service seam；迁移后的 `skill.sql_query` 新实现中，`llm.sql_query` 必须是主代理 LLMProvider 的非流式适配器，而不是 SQLQuery 专属 LLM client；历史过渡期内 native SQLQuery runtime 可暂时保持现状；当前完成态已删除 native runtime；
...
### 2.2 迁移前事实：SQLQuery 曾是 native capability

以下为迁移前事实，用于说明本计划要删除的旧实现；2026-05-13 完成态中这些 native 文件与符号已删除：
...
### 2.3 迁移前事实：orchestration 曾有 SQLQuery 业务耦合

以下为迁移前待清理项；2026-05-13 完成态中 orchestration 不再直接 import / 注册 native SQLQuery：
...
### 2.4 迁移前事实：API runtime 曾直接装配 SQLQuery

`src/api/runtime.py` 迁移前曾：
...
这些在完成态已删除；不再保留旧 request alias。

### 2.5 迁移前事实：前端曾依赖 native SQLQuery 标识

迁移前前端曾有多处 SQLQuery capability id / internal node id 判断：
...
→ registered skill.sql_query.platform_handler calls SQLQuery domain engine
...
skill_platform_handlers={
    "skill.sql_query.platform_handler": sql_query_platform_handler,
}
trusted_skill_handlers={
    "skill.sql_query": "skill.sql_query.platform_handler",
}
...
SQLQuery 迁移为 Skill 形态后，`skill.sql_query` 的 platform handler / domain engine 内部 LLM 调用必须遵守以下规则；2026-05-13 完成态已删除 native SQLQuery runtime：

1. **不再维护 SQLQuery 专属 LLM client。**
   `skill.sql_query` platform handler 及其 domain engine 不应自行创建、持有或读取独立的 SQLQuery LLM client / provider 配置；LLM service 由主代理 runtime 的非流式适配器提供。
2. **沿用主代理 LLMProvider。**
...
2. 显式 `skill.sql_query` 入口可用；旧 `sql_query.query` / `sql_query` 请求应被拒绝。
...
过渡期可以让 native `SQLQueryExecutor` 先改成调用 domain engine；完成态必须删除该 executor 和 `sql_query.query` public macro，对外只保留 `skill.sql_query`。
...
### Phase 2：新增 `skill.sql_query` platform-service handler（历史过渡阶段，已完成）

#### 目标

历史过渡阶段曾要求在删除 native SQLQuery 前，让 `skill.sql_query` 通过 Skill Executor 执行完整 SQLQuery 主链路并做对比；2026-05-13 完成态已删除 native capability，当前只保留 `skill.sql_query`。

#### 已落地模块

```text
src/sql_query/platform_handler.py
src/sql_query/engine.py
src/sql_query/<stage>.py
skill/sql-query/SKILL.md
configs/sql_query/            # 当前配置权威源
```

完成态继续复用仓库级 `configs/sql_query/` 作为 SQLQuery 配置权威源；未在 Skill 包内复制配置，因此不存在双源同步规则。

#### handler 职责

`skill.sql_query.platform_handler` platform handler 负责：
...
- `skill.sql_query` 输出与迁移前 SQLQuery 主路径结果等价；
...
让自动规划和显式路由只使用 `skill.sql_query`；旧 `sql_query` / `sql_query.query` 请求不再 canonicalize，作为 unsupported capability fail closed。
...
不再在 API 请求边界转换；应返回 unsupported capability 错误。当前唯一合法公开入口是：
...
- 显式旧 id 请求在 API 边界返回 unsupported capability，不再转换；
...
- `skill.sql_query.platform_handler` platform handler 注册；
- 旧 API alias 拒绝逻辑；
- MySQL readonly config 解析；迁移后的 `skill.sql_query` 不再解析或维护独立 LLM client config，只接收由主代理 LLMProvider 派生的非流式 `thinking=false` LLM service。当前完成态不再保留 native SQLQuery runtime。
...
2. `frontend/src/domain/taskEvents.ts`
   ...
   - 不再根据旧 internal capability id 推断当前进度；历史任务展示只能依赖已持久化 artifact metadata 或通用任务状态。
...
2. API 覆盖旧 `sql_query.query` 请求被拒绝；
3. 前端不再主动发送旧 id；
...
- `src/capabilities/sql_query/` 删除；SQLQuery 领域代码保留在 `src/sql_query/`；
...
- [x] `/api/v1/capabilities` 包含 `skill.sql_query`。
- [x] `/api/v1/capabilities` 不包含 `sql_query.query`。
- [x] `/api/v1/capabilities` 不包含任何 `sql_query.*` internal capability。
- [x] MCP tool 仍以 `mcp.*` 形式出现。
- [x] 其他项目 Skill 仍以 `skill.*` 形式出现。

### 6.2 orchestration 解耦

- [x] `src/orchestration/` 不再 import `src.capabilities.sql_query`。
- [x] `src/orchestration/` 不再 import `src.sql_query`。
- [x] Planner prompt 不再写死 `sql_query.query`。
- [x] Router 不再有 SQLQuery 专属分支。
- [x] Auto fallback 不再硬编码 SQLQuery。

### 6.3 SQLQuery 功能

- [x] 自然语言数据库问题仍能自动调用 `skill.sql_query`。
- [x] SQLQuery 内部 LLM 路由仍可用。
- [x] SQLQuery 内部 LLM 沿用主代理 LLMProvider，非流式、`thinking=false`，并忽略 / 不处理 `reasoning_content`。
- [x] SQL 生成仍可用。
- [x] SQL Guard 仍强制执行。
- [x] MySQL readonly 执行仍可用。
- [x] 结果筛选仍可用。
- [x] raw preview 与 filtered preview artifact 仍可用。
- [x] 最终助手回答仍可用。

### 6.4 安全

- [x] 普通 `python_subprocess` Skill 不能访问 MySQL readonly。
- [x] 普通 Skill 不能访问 SQLQuery 专用 LLM service binding；该 binding 只是主代理 LLMProvider 的受控非流式适配器。
- [x] `skill.sql_query` 只能通过 runtime allowlist handler 使用受控 services。
- [x] handler 未注册、service 未注册、service 未授权时 fail closed。
- [x] audit 不记录数据库连接串、账号、密码、LLM key、完整 prompt。
- [x] SQL Guard 失败不能被 Skill / handler 绕过。

### 6.5 兼容

- [x] `capability_id="sql_query.query"` / `"sql_query"` 不再被转换，API 边界返回 unsupported capability。
- [x] 前端默认发送 `skill.sql_query` 或完全交给自动规划。
- [x] 老 SQLQuery 结果卡片展示不退化。
- [x] 老任务事件 / 老 artifact 仍有 fallback 展示。
...
conda run -n multi_agent python -m unittest discover -s tests/sql_query -p 'test_*.py'
...
| 旧 alias 长期存在造成概念混乱 | 已消除 | API 边界拒绝 `sql_query` / `sql_query.query`，唯一公开入口为 `skill.sql_query` |
...
落地顺序已于 2026-05-13 收敛为完成态：

1. ✅ baseline / domain engine：SQLQuery 行为测试迁移到 `tests/sql_query/`，领域实现收口到 `src/sql_query/`。
2. ✅ platform handler：`skill/sql-query/SKILL.md` 声明 `skill.sql_query.platform_handler`，runtime 通过 allowlist 注册 handler/services。
3. ✅ skill e2e：显式 `skill.sql_query`、自动规划、guard blocked、finalizer、artifact metadata 全链路通过。
4. ✅ routing cleanup：Planner / Router / AutoWorkflowProvider / Replanner 去除 SQLQuery native 特判。
5. ✅ runtime cleanup：API runtime 不再注册 native SQLQuery descriptor、instance、provider、executor、replanner 或 macro provider。
6. ✅ frontend：前端提交 `skill.sql_query`，以 metadata / stage event 识别 SQLQuery 结果。
7. ✅ native removal：删除 `src/capabilities/sql_query/` 与旧专项 native 测试。
8. ✅ alias removal：`sql_query` / `sql_query.query` API 请求不再转换，返回 unsupported capability。
...
2. **LLM Provider 复用规则约束当前 `skill.sql_query` 实现：复用主代理 LLMProvider 的非流式适配，不再维护 native SQLQuery runtime。**
3. **SQLQuery 业务逻辑位于 `src/sql_query/` domain engine，并由 Skill handler 调用。**
4. **orchestration 已完成去耦合，只能看到 `skill.sql_query` 等公开 capability。**
5. **前端以 `skill.sql_query`、`domain_kind=sql_query` 和 stage event 识别当前 SQLQuery；旧字符串只作为历史 artifact/event 展示 fallback。**
