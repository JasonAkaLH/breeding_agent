...
`docs/prd/backend/06-SQLQuery-MVP设计.md` 定义了一期 SQLQuery 只读链路；随后实现扩展到 LLM 驱动的 SQL 生成、LLM 结果筛选、真实 MySQL 只读适配器与 prompt schema 约束。当前默认产品链路已迁入 `skill.sql_query` domain engine，尾阶段 result filtering 在 SQLQuery 内部对 LIKE 召回的候选行做二次筛选并返回筛选后的表格，同时 readonly execute 阶段保留原始表格 preview；本 PRD 记录这些能力的维护边界。
...
- 通过真实 MySQL 只读查询 smoke 验证数据库适配器与 `skill.sql_query` 链路可用。
...
SQL generation 阶段支持注入最小文本生成接口；真实 API runtime 默认通过 `skill.sql_query` platform-service 的受控 LLM service 复用主代理 LLMProvider 非流式 adapter，并按照以下契约处理 LLM 输出：
...
schema context prepare 阶段不再按自然语言问题用规则函数预先挑选业务字段；它只负责在已确定 route / crop / table 范围内暴露所有 `expose_to_llm: true` 的字段（以及必要 join 字段）。SQL generation 阶段的 LLM 必须自行从 `selected_column_details` 中选择 SELECT 投影字段、WHERE 过滤字段和排序字段；例如用户问“适合河南种植”时，应优先选择描述为“适种区域”的 `suitable_area` 并生成类似 `suitable_area LIKE '%河南%'` 的过滤条件。SQL Guard 仍负责最终只读、安全、表字段白名单校验。
...
- 后续仍必须通过 `skill.sql_query` 内部 SQL guard 阶段，LLM 自身不能绕过 guard；Guard 表白名单优先使用 `selected_tables`（即当前 prompt 实际注入表），缺失时才回退 route `allowed_tables`。

### 4.4 Runtime 装配

`build_api_runtime()` 为 SQLQuery Skill 提供受控 LLM service 装配：
- `llm_text_generator`：最高优先级，测试或特殊运行时可直接注入文本生成函数；
- SQLQuery Skill 通过通用 `llm.non_stream` platform service 获取非流式 LLM adapter，底层复用主代理 `SharedLLMRuntime` 的非流式 adapter；
- SQLQuery 内部调用固定 `thinking=False` 且只消费最终 answer；不再维护独立 SQLQuery LLM client / runtime；
- `enable_sql_query_llm`：可在 fake backend / 默认自动化测试中显式关闭真实 provider 访问。

同一个 resolved text generator 会在 SQLQuery Skill handler 内服务 intent route、SQL generation 与 result filtering，确保具体数据库 route 判断、SQL 生成和候选结果筛选都先走 LLM 主路径，再按各自规则降级。`intent_route` 的 LLM 固定使用非流式 `generate_text` 且强制 `thinking=False`，不继承前端深度思考开关；它只允许返回已配置的 `route_id`，不生成 SQL。输出非法、provider 失败或 route 不在配置集合内时，回退到原有配置规则路由。

高层 Planner 只能选择 public Skill 能力 `skill.sql_query` 与高层依赖关系，不能通过 `route_hint` 指定具体数据库；`SkillWorkflowProvider` 只负责展开通用 Skill executor，具体 route 判断由 SQLQuery Skill handler 内部的轻量 LLM 阶段完成。复合查询拆出的子问题可以携带 `subtask_label` / `parent_question` 作为上下文，但每个子问题最终仍由 SQLQuery 内部轻量 LLM 路由阶段判断应查哪个库。
...
同时写入 audit-only 的 `skill.llm_fallback` 事件，并在 payload 中标记 `capability_id=skill.sql_query` 与 `stage=sql_generate`；成功调用写入 `skill.llm_call`。

## 5. 结果筛选与表格回传契约

当前默认 SQLQuery Skill domain engine 的尾阶段是 `result_filtering`。SQL 生成阶段使用 `LIKE` 匹配品种名是为了先召回候选行，避免差一个字、简称或后缀导致漏查；`result_filtering` 只负责在已执行结果上判断哪些候选行真正符合用户需求，不重新生成 SQL、不修改 SQL、不补查数据库、不生成自然语言总结。
...
- `satisfaction`：机器可读的结果满足度，包含 `satisfied`、`reason_code`、`message` 与 `replan_recommended`，供通用 runtime replanner 判断是否需要把多子需求拆成多个 public `skill.sql_query` 节点。
...
当禁用/未配置 LLM 或 0 行结果时，节点走确定性路径并保留 SQL 返回的候选表格；当 provider 失败、JSON 非法或 `keep_row_indexes` 越界/类型非法时，节点回退为未筛选表格（再叠加确定性后过滤规则），并写入 `skill.llm_fallback` audit 事件，payload 标记 `capability_id=skill.sql_query` 与 `stage=result_filtering`。成功调用写入 `skill.llm_call`。
...
- `skill.llm_call`（payload 标记 `capability_id=skill.sql_query` 与具体 `stage`）
- `skill.llm_fallback`（payload 标记 `capability_id=skill.sql_query` 与具体 `stage`）
...
- `skill.llm_call` / `skill.llm_fallback` 不记录完整 prompt、完整 rows 或 secret；
