...
## 10.5 SQLQuery MVP 任务链路细化草案

> 当前 SQLQuery 以 `skill.sql_query` platform-service 公开；以下六段是 project Skill bundle 内 `runtime/sql_query_skill/` 调用的 domain stages。公开 capability 只有 `skill.sql_query`。

### 10.5.1 首阶段建议 Domain Stage 拆分
一期 SQLQuery MVP 建议至少拆成以下内部阶段：
- intent route：识别用户是否为数据库查询意图，并提取查询目标
- schema context prepare：获取可用 schema、表、字段、必要约束信息
- SQL generation：基于自然语言与 schema 生成 SQL 草案
- SQL guard：校验 SQL 是否只读、是否单语句、是否命中禁用模式
- readonly execution：执行只读 SQL 并返回结构化结果
- result filtering：对 SQL LIKE 召回的候选结果做二次筛选并返回筛选后的表格

其中：
- SQL guard、readonly execution 是 domain engine 内的强制安全阶段，不作为外部 capability 暴露；
- SQL generation 可由受控 LLM service 支撑；
- result filtering 可由外部 LLM 判断候选行是否符合用户需求，失败时保守保留候选表格。
...
### 10.5.3 动态扩展节点
在混合型 DAG 模型下，一期允许有限动态扩展，但必须受控。SQLQuery 场景建议只允许在 handler 内部以下类型的受控重试/补充：
...
### 10.5.5.1 严格只读防线（必须项）
`skill.sql_query` 在一期必须被定义为**严格只读能力**，不能通过任何执行路径写入数据库。建议采用至少四层防线：

1. **Skill 合约层**
   - `skill.sql_query` 明确定义为 `read_only_query_only`
   - 不存在“查询失败后自动切换为写入型 SQL”的回退路径

2. **SQL Guard 层**
   - 所有 SQL 在进入 readonly execution 阶段前必须经过 SQL guard 阶段
   - 只允许 `SELECT` 与只读 `WITH ... SELECT`
   - 任何无法确定是否安全的 SQL，一律拒绝执行
   - 多语句、DDL、DML、锁表、导入导出语句一律拦截

3. **执行适配层**
   - `skill.sql_query` domain engine 内的 readonly execution 阶段只能接收 guard 已通过的 SQL
...
- 不执行 SQL
- 前端可见任务收敛为 `task.failed` / `node.failed`，错误 payload 只包含安全 block code
- 写入 JSONL 审计日志；审计事件可使用 SQLQuery domain namespace，但不是 public capability 或前端进度事件
- 返回明确的能力边界说明：当前 SQLQuery Skill 仅支持只读查询
...
除通用事件外，一期建议通过 `skill.progress` 输出可见进度，并在 payload 中携带 `domain_kind=sql_query`、`capability_id=skill.sql_query` 与内部 `stage`：
- intent route
...
审计事件使用 `skill.*` event type，并通过 `capability_id=skill.sql_query` 与 `stage` payload 标记 SQLQuery 领域上下文。
...
### 10.6.1 目标
Schema Context Builder 负责在 intent route 阶段完成后，根据路由结果、schema profile、用户问题和任务约束，生成一份**最小必要 schema 上下文**提供给 SQL generation 阶段，而不是把整库 schema 原样发送给 LLM。
...
该输出会作为 `schema_context_snapshot` artifact 持久化，并作为 `skill.sql_query` 内部 SQL generation 阶段的核心输入之一。
...
### 10.6.11 建议事件
一期建议补充以下事件：
- `skill.progress` with `stage=schema_context_prepare` / `stage=sql_generate`
- audit-only schema context build events with `capability_id=skill.sql_query` and explicit `stage` payload
