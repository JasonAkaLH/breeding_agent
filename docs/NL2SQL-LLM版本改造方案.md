...
- 状态：提案 / 待进入实施
- 日期：2026-04-23
- 适用范围：在当前 Phase 5 已完成的 NL2SQL MVP 基础上，升级为真正使用 LLM 的版本

---

## 1. 背景

当前仓库里的 NL2SQL capability 已经形成一条**可运行的 MVP 闭环**：
...
- `schema_context_prepare` 已复用现有 `src/nl2sql/schema_context_builder.py`
- `sql_guard` 已接入 `configs/nl2sql/sql_guard_rules.yaml`
- `sql_execute_readonly` 已通过 `src/integrations/mysql_readonly.py` 形成只读执行适配层

但当前版本的两处关键能力仍是**启发式 / stub-friendly**实现，而不是真正的 LLM 版本：

1. `src/capabilities/nl2sql/sql_generate.py`
   - 当前默认通过规则拼接 SQL
2. `src/capabilities/nl2sql/result_summarize.py`
   - 当前默认通过确定性模板生成结果摘要
...
- `src/capabilities/nl2sql/workflow.py`
  - 已定义标准 6 节点 workflow
- `src/capabilities/nl2sql/executor.py`
  - 已提供 `sql_generator` / `summarizer` 注入位
- `src/capabilities/nl2sql/schema_context_prepare.py`
  - 已能产出 route / selected_tables / selected_columns / join_hints / context_summary
- `src/capabilities/nl2sql/sql_guard.py`
  - 已能执行只读安全校验并发放 `guard_pass_token`
- `src/integrations/mysql_readonly.py`
  - 已能在 async 边界后执行只读 SQL
- `llm_client.py`
  - 已有异步 OpenAI client 雏形

### 2.2 当前能力短板

#### SQL 生成
当前 `sql_generate` 的默认行为是：
- 选择 1~2 张表
- 从裁剪后的字段里拼接 `SELECT`
- 用 join hint 拼接 `JOIN`
- 默认补 `LIMIT 50`
...
- 重写 `src/nl2sql/schema_context_builder.py`
- 取消或弱化 `sql_guard`
...
- `executor.py`
  - 正式接入 `llm_client.py`
  - 注入真实 `sql_generator` 与 `summarizer`
...
- `src/capabilities/nl2sql/prompt_builders.py`
...
3. 让 `NL2SQLExecutor` 能在“测试模式 / 真实 LLM 模式”之间切换
...
- `src/capabilities/nl2sql/sql_generate.py`
...
- `src/capabilities/nl2sql/result_summarize.py`
...
在现有：
- `tests/capabilities/nl2sql/test_sql_generate.py`
- `tests/capabilities/nl2sql/test_result_summarize.py`
...
4. **闭环测试**
   - 一条基于 orchestration 的 NL2SQL LLM 版闭环测试
...
#### 缓解
- 在 `llm_client.py` 层做 timeout / retry
...
1. `sql_generate` 默认走 LLM 主路径
2. `sql_generate` 在 LLM 失败时能降级
3. `result_summarize` 默认走 LLM 主路径
...
- `tests/capabilities/nl2sql/test_sql_generate.py`
- `tests/capabilities/nl2sql/test_result_summarize.py`
- `tests/capabilities/nl2sql/test_orchestration_flow.py`
