...
- **文档状态**：草案第八版
- **日期**：2026-04-22
- **本版目标**：在 v6 基础上明确内部多 Agent 协作协议，补充结构化 mailbox 与 interrupt/resume 设计
...
## 15. 当前仍待定事项
以下内容仍需在后续版本继续细化：
- Schema context builder 的实现方式与缓存策略
- PostgreSQL 部署完成后的状态库存储详细设计（当前本地测试先用 SQLite）
- 数据库查询取消能力、只读账号管理与外部 LLM cancel 契约的统一规范
- 任务优先级、配额与背压策略
- DAG 动态扩展的具体约束边界
- 结构化 mailbox 的落表结构与 ACK / TTL 细节
