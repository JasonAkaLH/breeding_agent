...
Update the Rust core contract snapshot so it no longer declares legacy auth models. Update its schema hash after the snapshot changes.
...
Delete repository documents whose main subject is the replaced auth model:
...
- `AuthUser`, `CaptchaChallenge`, `AuthSession`, and `AuthApiToken` are absent from the contract snapshot.
...
```bash
rg -n "__Host|maf_session|Cookie|cookie|captcha|register|api-tokens|required_scopes|AuthSource|AuthSession|AuthApiToken|CaptchaChallenge|AuthUser" src frontend/src docs/api docs/superpowers .omx/plans tests native
```
