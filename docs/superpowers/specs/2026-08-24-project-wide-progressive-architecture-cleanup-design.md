...
- **日期**：2026-08-24
- **状态**：设计章节已获用户逐项批准；document-perfectization 第 7 轮审计与修订完成，待第 8 轮独立复审
- **审查轮次**：7 次完整审计、7 次授权修订
- **适用分支**：`main`
- **适用仓库**：`breeding_agent`
...
| NFR-ENV-01 | 验证隔离 | ordinary gate 必须从空 env mapping 构造 closed profile 与 temp home；external gate 只注入具名 closed keys 并在连接前证明 non-prod/独立 scope；artifact只能进入profile-bound `checkout|runner_temp` root且绝对root不进evidence；未声明部署/DSN/credential/proxy不得继承 | P0/全部；`gate-environment-profile.json`、launcher root/env tests/preflight |
...
- `gate-environment-profile.json`：ordinary/PG/MCP/Ubuntu 各 gate 的 `evidence_phase`、artifact root kind/relative names、允许传入与launcher-owned internal env key、禁止 key/prefix、新鲜 temp home/cache 与非生产预检；
...
P0 新增 `scripts/run_result_parser_linux_gate.py`，不解析 console 文本：它在两个隔离子进程中用 `unittest.TestLoader`/`TestResult` 程序化加载 exact targeted suite 与 full MCP integration suite，接收 barrier fixtures 的脱敏 RLIMIT/terminate/join/kill/process-final-state collector，并原子写入 profile root 下 `result-parser-linux-gate.v1.json`。Schema 固定为 `maf.cleanup.result_parser_linux_gate.v1`，包含 canonical SHA、actual HEAD/tree、env artifact binding、runner/kernel/Python/pip/requirements digest、target/full discovered/ran/failure/error/skip、RLIMIT before/after、timeout 与 process-cleanup trace，禁止 raw 64 MiB payload。`P3.ubuntu-result-parser` 固定 `artifact_root_kind=runner_temp`、`artifact_root_relative_dir=progressive-cleanup-result-parser-linux`，env/result filename exact；launcher按11.1在allowlisted `RUNNER_TEMP` 下创建该private root，Runner双验immutable env binding后，workflow才可upload。Job唯一命令为：
...
P0 新增 repo-owned `scripts/run_rust_fuzz_smoke_gate.py --output <path>`。`P7.ubuntu.rust-fuzz-smoke` 固定 `evidence_phase=pre_spawn_immutable`、`artifact_root_kind=checkout`、root relative dir=`docs/checkpoint/progressive-architecture-cleanup/gate-results`，env/result filenames分别为`rust-fuzz-smoke-env.v1.json`与`rust-fuzz-smoke.v1.json`。唯一执行命令必须由通用launcher包裹runner；runner先按11.1验证env binding，再从P0 closed target-owner map选择`artifact_path|skill_runtime_policy|data_access_readonly|audit_sanitizer|auth_core|mcp_runtime_protocol`中本检查点适用项，以argv直接执行每项fixed 30s command，并为schema=`maf.cleanup.rust_fuzz_smoke.v1`的result记录env SHA、`tested_commit/tree`、target、canonical command、duration、exit、conclusion、Rust toolchain、cargo-fuzz version及只含size/count/digest的脱敏corpus metadata；原子写入后自校验target set/schema/digest。Workflow必须验证并上传env+result两个fixed files；没有适用target时只能由owner map产出有理由的N/A，适用target缺失、重复、未运行或任一artifact未上传均使leaf失败。固定命令为：
...
Required external env 名固定为：`CLEANUP_REAL_MCP_ENDPOINT`、`CLEANUP_REAL_MCP_PROTOCOL_VERSION`、`CLEANUP_REAL_MCP_CONTROL_ENDPOINT`、`CLEANUP_REAL_MCP_CONTROL_HOST`、`CLEANUP_REAL_MCP_CONTROL_SPKI_SHA256`、`CLEANUP_REAL_MCP_CONTROL_TOKEN`、`CLEANUP_REAL_MCP_ORDINARY_TOOL`、`CLEANUP_REAL_MCP_APPROVAL_TOOL`、`CLEANUP_REAL_MCP_ARTIFACT_TOOL`、`CLEANUP_REAL_MCP_RESPONSE_LOSS_TOOL`；profile-owned child constant 另为 `CLEANUP_REAL_MCP_RUN=1`。`P3.real-mcp` 固定 `evidence_phase=pre_spawn_immutable`、`artifact_root_kind=checkout`、root relative dir=`docs/checkpoint/progressive-architecture-cleanup/gate-results`及命令中的env/result filenames。Launcher按11.1先写并冻结schema=`maf.cleanup.gate_environment.v1`的env artifact，再通过四个launcher-owned root/binding internal keys传递，之后才spawn。Artifact记录root kind/relative names、external env key presence、允许的非敏感digest、profile definition/child argv digest与tested commit/tree，不记录runtime绝对root、endpoint、token或credential；它不得写或merge业务result artifact，child结束后也不得改写。

P0 新增 repo-owned `run_controlled_real_mcp_gate.py`，它在任何control/MCP连接前从internal keys读取root kind/runtime root/relative env name+expected SHA，验证root containment、schema、commit/tree、profile/argv digest与canonical SHA；写result前再次验证同一inode/mode/size/SHA。随后程序化装载exact五个test、拒绝额外/缺失/skip，用harness-owned collector汇总domain evidence，并以temp file+fsync+replace原子写schema=`maf.cleanup.controlled_real_mcp_result.v1`的result artifact后立即重读验证。`--output`是唯一业务evidence channel且必须等于declared root+result filename；unittest、launcher与test case均不得各自覆盖同一路径。最终validator必须分别验证env/result两份artifact的`tested_commit/tree`、root kind/relative names、schema、canonical digest、profile/result binding与5/5 outcome，任一缺失、双写、root替换/escape、错误digest、运行中或运行后env mutation均fail。
...
Result evidence `gate-results/controlled-real-mcp-result.v1.json` 必须按 11.1 canonicalization 记录 `tested_commit/tree`、启动前冻结且两次验证的env artifact root kind+relative name+digest、profile/child-argv digest、`relevant_source_digest`、protocol/version/adapter、Server ref digest、5个场景ID、脱敏Task/Call ref digest、本地arguments SHA与Server final-input digest关联、每阶段request/effect/commit counter before/after、transport-send与按side-effect/operation分类的restart delta、projection/artifact schema+digest、setup/business/cleanup timeout、TTL/skew、redaction scan与cleanup/revoke result，禁止runtime绝对root。`gate-results/controlled-real-mcp-env.v1.json`只承担immutable pre-spawn launcher/profile evidence；最终gate artifact set必须保存、校验并上传两者。
...
| R-31 | 通用pre-spawn artifact root与CI `RUNNER_TEMP`/checkout布局冲突或泄漏绝对路径 | profile-bound root kind、private 0700 runner子目录、dirfd/no-follow containment、root-swap fixtures | active |
