...
- **日期**：2026-08-24
- **状态**：设计章节已获用户逐项批准；document-perfectization 第 8 轮审计与修订完成，待第 9 轮独立复审
- **审查轮次**：8 次完整审计、8 次授权修订
- **适用分支**：`main`
- **适用仓库**：`breeding_agent`
...
| NFR-ENV-01 | 验证隔离 | ordinary gate必须从空env构造closed profile/temp home；external只注入具名keys并先证non-prod；cwd/canonical argv必须profile-bound，artifact只能进入`checkout|runner_temp` root，raw argv/绝对root不进evidence/log；未声明部署/DSN/credential/proxy不得继承 | P0/全部；environment profile、launcher root/cwd/argv/env tests |
...
- `gate-environment-profile.json`：ordinary/PG/MCP/Ubuntu 各 gate 的 `evidence_phase`、typed canonical argv、relative child/subcommand cwd、artifact root kind/names、allowed ignored-output policy、允许传入与launcher-owned internal env key、禁止 key/prefix、新鲜 temp home/cache 与非生产预检；
...
JSON artifact 必须包含 `schema_version`、`baseline_commit`、`baseline_tree`、11.1定义的canonical command/cwd及`canonical_sha256`。摘要算法固定为：移除顶层`canonical_sha256`字段后，使用锁定Python版本的`json.dumps(value, ensure_ascii=False, sort_keys=True, separators=(",", ":"), allow_nan=False)`，再UTF-8 encode并计算SHA-256；输入number类型不得先做字符串化/float归一，NaN/Infinity必须拒绝。校验器必须重算，禁止把摘要字段本身纳入摘要。不得保存credential、DSN、用户正文、Tool参数、raw result、附件正文、runtime artifact root或其他绝对敏感路径。
...
`gate-environment-profile.json` 为每个 profile 固定 `evidence_phase=post_exit|pre_spawn_immutable`、`child_cwd_relative`（默认`.`）、`canonical_command_argv`与`allowed_ignored_output_roots`。普通、没有child-owned result的gate使用`post_exit`；Result Parser、Rust fuzz与controlled real-MCP等有独立result producer的profile使用`pre_spawn_immutable`，并必须声明`artifact_root_kind=checkout|runner_temp`、由非空安全component组成的规范相对`artifact_root_relative_dir`、单段env artifact filename、closed child-result filename set与fixed file mode（checkout非敏感证据=`0644`，private runner-temp=`0600`）。

`checkout` 以 disposable checkout realpath 为 base，允许使用commit内已存在但逐component非symlink的相对目录；目标若不存在可创建，若已存在则必须是该commit已跟踪的exact evidence path、regular/single-link/owner/mode正确并记录prior digest，随后才允许同目录exclusive temp file+fsync+atomic replace，不能原地truncate；Gate后tracked diff只能含closed filename set。`runner_temp` 只允许 repo-owned CI profile显式读取 infrastructure key `RUNNER_TEMP`，launcher不把宿主该变量原样继承给child，而是在其已lstat/realpath/owner验证、非symlink的base下创建profile固定且原先不存在的`0700`子目录，所有target必须原先不存在。两类root都拒绝空/绝对/`..`/分隔符逃逸、symlink component、root-kind替换、未知预存目标和不匹配的`--evidence-output|--output`；artifact writer统一使用dirfd+no-follow、exclusive temp与atomic replace，final mode按profile固定。`runner_temp` profile在artifact中只记录root kind、相对目录/文件名、owner/mode/inode/size/digest等非敏感metadata，绝不记录runtime绝对root。

Profile 的`child_cwd_relative`必须由安全relative components组成并解析到disposable checkout内已存在、owner正确、逐component非symlink的directory；launcher忽略caller cwd，始终把该resolved directory显式传给`subprocess`的`cwd=`，artifact只记录relative cwd及其mode/inode，不记录checkout绝对root。Backend/ordinary/P3 runner/P4/P5/P7 quality/real-MCP默认为`.`；Frontend固定`frontend`。Repo-owned child runner若再spawn子命令，必须为每类subcommand另有closed relative cwd并执行同一验证，禁止继承runner或caller cwd。

`canonical_command_argv`是typed token vector：普通参数为`{kind:"literal",value:<exact-safe-token>}`；当前revision参数为`{kind:"tested_commit"}`，actual必须等于launcher已验证的40-hex SHA且canonical artifact写入该safe value；artifact path参数为`{kind:"artifact_path",root_kind,relative_dir,filename}`；禁止把expanded runtime path当literal。Launcher将实际expanded argv中的declared artifact path验证为resolved root下exact filename后投影成typed token，tested-commit与literal token也必须逐项exact；所得vector必须与profile token schema exact，按11.1 JSON算法另算`canonical_command_sha256`。本文所有“生成/实际命令”均指这个已由actual argv验证的canonical vector+digest，不指raw expanded argv。Launcher不得echo/persist raw argv；child stdout/stderr通过pipe进入longest-first exact-byte streaming redactor，覆盖checkout、artifact root及其runner-temp base、temp home与dependency-cache runtime roots，以`max(root-byte-length)-1` carry处理跨chunk sentinel，分别替换为closed `$CHECKOUT_ROOT|$ARTIFACT_ROOT|$RUNNER_TEMP|$TEMP_HOME|$DEPENDENCY_CACHE` token后才输出或保存；exception也走同一safe formatter，raw chunk不落盘。P0 fixtures必须证明两个不同absolute`RUNNER_TEMP`得到同一canonical vector structure（仅tested commit按输入变化）且无root-dependent digest、各root sentinel即使跨chunk也在artifact/已发布stdout/stderr/exception中为0，以及伪造template、commit、cwd或越界actual output在spawn前失败。

Launcher 必须先验证 profile/HEAD/tree/canonical argv、child cwd与artifact root，原子写入+fsync+重读 env artifact并取得canonical SHA，再注入 launcher-owned、禁止由宿主提供或日志输出的 `CLEANUP_GATE_ARTIFACT_ROOT_KIND`、`CLEANUP_GATE_ARTIFACT_ROOT`（runtime绝对root，仅child可见）、`CLEANUP_GATE_ENV_ARTIFACT_NAME`（单段relative filename）和 `CLEANUP_GATE_ENV_ARTIFACT_SHA256`，最后才spawn。Env artifact只记录这四个internal key的名称、生成算法和“由launcher后注入”，不记录尚未生成的runtime root/SHA值，避免摘要自引用；它不含child outcome，spawn后永久只读且launcher不再改写，child outcome只进入child result。每个repo-owned child result runner都必须验证root kind、root realpath/owner/mode、`join(root, relative_name)` containment，并在任何测试/连接/副作用前及result原子写前两次验证同一path/inode/mode/size/schema/commit/tree/SHA，把`root_kind+relative_name+env SHA`写入result而非绝对path。Launcher在child exit后第三次验证同一entry。P0 fault tests覆盖env未完成即spawn、缺/错root kind/path/digest、checkout↔runner-temp替换、`..`/symlink/root swap、cwd替换、运行中/运行后mutation；任一情形fail，且外部连接/业务副作用在首次child验证失败时为0。
...
P0 新增 `scripts/run_result_parser_linux_gate.py`，不解析 console 文本：它在两个隔离子进程中用 `unittest.TestLoader`/`TestResult` 程序化加载 exact targeted suite 与 full MCP integration suite，接收 barrier fixtures 的脱敏 RLIMIT/terminate/join/kill/process-final-state collector，并原子写入 profile root 下 `result-parser-linux-gate.v1.json`。Schema 固定为 `maf.cleanup.result_parser_linux_gate.v1`，包含 canonical SHA、actual HEAD/tree、typed canonical command/relative cwd、env artifact binding、runner/kernel/Python/pip/requirements digest、target/full discovered/ran/failure/error/skip、RLIMIT before/after、timeout 与 process-cleanup trace，禁止 raw argv、runtime root或64 MiB payload。`P3.ubuntu-result-parser` 固定 `artifact_root_kind=runner_temp`、`artifact_root_relative_dir=progressive-cleanup-result-parser-linux`、`child_cwd_relative=.`，env/result filename exact；launcher按11.1在allowlisted `RUNNER_TEMP` 下创建该private root，Runner双验immutable env binding后，workflow才可upload。Job唯一命令为：
...
python scripts/run_progressive_cleanup_gate.py \
  ...
  --evidence-output "$RUNNER_TEMP/progressive-cleanup-result-parser-linux/result-parser-linux-env.v1.json" \
  -- python scripts/run_result_parser_linux_gate.py \
     --tested-commit "$TESTED_COMMIT" \
     --output "$RUNNER_TEMP/progressive-cleanup-result-parser-linux/result-parser-linux-gate.v1.json"
```

Targeted profile 的 two test IDs 由 runner 内部常量锁定，expected count 恰为 2、`ran=2`、`failed=0`、`errors=0`、`skipped=0`；MCP integration full suite 也必须记录 discovered/ran/failure/error/skip，且 skip=0。Job 只从上述 private root 上传 `progressive-cleanup-result-parser-linux` artifact 的两个固定文件（env profile + runner result）；upload 前再次验证 root kind/relative names、无第三个entry、无symlink/escape与两文件digest，artifact内容不得保存`RUNNER_TEMP`绝对path。缺文件/字段/脱敏检查或校验不符即 fail。`workflow_dispatch` 新增 required `tested_commit` input，job checkout 该 exact SHA 并验证 actual HEAD；push 事件只有 actual `github.sha == tested_commit` 时可闭合，pull-request 默认 merge-ref artifact 只是 preflight，不能闭合 implementation commit gate。只有 artifact commit 等于 `tested_commit` 且 job success 才闭合 `P3.ubuntu-result-parser`；runner/job/artifact 缺失时该 Result Parser 子检查点和 P3 总体为 `pending_platform`。P3 其他独立检查点可继续，但不得用 macOS skip 或既有 wheel job冒充通过。
...
| Result Parser | first-key/semantic-validation阶段、pre/post-checkpoint及三个cleanup-join cancel、process cleanup、gate/size/timeout、spawn identity exact；Ubuntu private runner-temp root内env/result两artifact、全量零skip、无absolute-path泄漏 | Result Parser barrier worker/service/root tests + `P3.ubuntu-result-parser` |
...
P0 新增 repo-owned `scripts/run_rust_fuzz_smoke_gate.py --output <path>`。`P7.ubuntu.rust-fuzz-smoke` 固定 `evidence_phase=pre_spawn_immutable`、`artifact_root_kind=checkout`、root relative dir=`docs/checkpoint/progressive-architecture-cleanup/gate-results`、runner `child_cwd_relative=.`，env/result filenames分别为`rust-fuzz-smoke-env.v1.json`与`rust-fuzz-smoke.v1.json`。唯一执行命令必须由通用launcher包裹runner；runner先按11.1验证env binding，再把每个cargo-fuzz subcommand的cwd固定为disposable checkout内`native/fuzz`，逐component验证realpath/owner/non-symlink，并验证该处`Cargo.toml`、`Cargo.lock`及六个target owner map digest；不得继承launcher、runner或workflow caller cwd。Runner从closed map选择`artifact_path|skill_runtime_policy|data_access_readonly|audit_sanitizer|auth_core|mcp_runtime_protocol`中本检查点适用项，以argv直接执行每项fixed 30s command，并为schema=`maf.cleanup.rust_fuzz_smoke.v1`的result逐target记录env SHA、`tested_commit/tree`、target、`command_cwd_relative=native/fuzz`、typed canonical argv/digest、duration、exit、conclusion、Rust toolchain、cargo-fuzz version及只含size/count/digest的脱敏corpus metadata；原子写入后自校验target set/schema/digest。P0 fixture从两个任意外层cwd调用launcher，必须得到相同relative cwd/command并由subprocess spy证明实际cwd exact；错误manifest root/symlink/caller-cwd inheritance在cargo前fail。Workflow必须验证并上传env+result两个fixed files；没有适用target时只能由owner map产出有理由的N/A，适用target缺失、重复、未运行或任一artifact未上传均使leaf失败。固定命令为：

```bash
python scripts/run_progressive_cleanup_gate.py \
  --profile P7.ubuntu.rust-fuzz-smoke \
  --evidence-output docs/checkpoint/progressive-architecture-cleanup/gate-results/rust-fuzz-smoke-env.v1.json \
  -- python scripts/run_rust_fuzz_smoke_gate.py \
     --output docs/checkpoint/progressive-architecture-cleanup/gate-results/rust-fuzz-smoke.v1.json
```
...
每一步的11.1 typed canonical command/digest与relative cwd、结果、test count、skip、platform、`start_commit`、`tested_commit`、disposable checkout identity/cleanup与artifact digest必须写入`docs/checkpoint/progressive-architecture-cleanup/gate-results/`；raw expanded argv/absolute cwd不得写入。Gate必须按11.1在`tested_commit`的clean disposable checkout上运行，原工作树可保留已登记的无关用户diff；随后evidence-only commit不得修改业务或测试源码。`contract diff`必须列出比较文件及SHA；`final diff review`必须记录reviewer、scope与结论，不能只写一句“已检查”。
...
Frontend canonical profile 固定 `child_cwd_relative=frontend`，launcher从任意caller cwd都显式以该目录执行下列child argv；`cd frontend`不是shell前置，也不进入canonical command。依赖materialization绑定`package-lock.json`、Node/npm版本和P0记录的只读content-addressed cache digest；launcher先把该cache复制到gate-local writable temp cache并只向npm注入该路径，随后用fixed offline argv在clone内生成writable ignored`node_modules/`，不得复用可写宿主依赖目录或访问registry。Canonical gate argv依次为：
...
Profile 的 `allowed_ignored_output_roots` closed set 是 `node_modules/`（dependency、`.tmp`、Vite/Vitest cache）、`public/vendor/`（pretest/prebuild MathJax）和`dist/`（build）；artifact-root约束只管理gate evidence，不把这些command outputs误判成evidence。Launcher在dependency setup前和每条command后以NUL-safe inventory验证三个root都在resolved `frontend/`下、top-level root非symlink，记录relative path/presence/mode/file-count/bytes和lock/cache digest，禁止任何其他ignored/untracked output或tracked diff；内部`node_modules`链接由lock/integrity验证，不复制其绝对target到artifact。恶意top-level symlink/escape、从repo root误跑、写到clone外或新增第四root均使gate失败；clone cleanup仍按11.1执行。
...
Required external env 名固定为：`CLEANUP_REAL_MCP_ENDPOINT`、`CLEANUP_REAL_MCP_PROTOCOL_VERSION`、`CLEANUP_REAL_MCP_CONTROL_ENDPOINT`、`CLEANUP_REAL_MCP_CONTROL_HOST`、`CLEANUP_REAL_MCP_CONTROL_SPKI_SHA256`、`CLEANUP_REAL_MCP_CONTROL_TOKEN`、`CLEANUP_REAL_MCP_ORDINARY_TOOL`、`CLEANUP_REAL_MCP_APPROVAL_TOOL`、`CLEANUP_REAL_MCP_ARTIFACT_TOOL`、`CLEANUP_REAL_MCP_RESPONSE_LOSS_TOOL`；profile-owned child constant 另为 `CLEANUP_REAL_MCP_RUN=1`。`P3.real-mcp` 固定 `evidence_phase=pre_spawn_immutable`、`artifact_root_kind=checkout`、`child_cwd_relative=.`、root relative dir=`docs/checkpoint/progressive-architecture-cleanup/gate-results`及命令中的env/result filenames。Launcher按11.1先写并冻结schema=`maf.cleanup.gate_environment.v1`的env artifact，再通过四个launcher-owned root/binding internal keys传递，之后才spawn。Artifact记录root kind/relative names、external env key presence、允许的非敏感digest、profile definition、typed canonical command/cwd digest与tested commit/tree，不记录raw argv、runtime绝对root、endpoint、token或credential；它不得写或merge业务result artifact，child结束后也不得改写。

P0 新增 repo-owned `run_controlled_real_mcp_gate.py`，它在任何control/MCP连接前从internal keys读取root kind/runtime root/relative env name+expected SHA，验证root containment、schema、commit/tree、canonical command/cwd digest与canonical SHA；写result前再次验证同一inode/mode/size/SHA。随后程序化装载exact五个test、拒绝额外/缺失/skip，用harness-owned collector汇总domain evidence，并以temp file+fsync+replace原子写schema=`maf.cleanup.controlled_real_mcp_result.v1`的result artifact后立即重读验证。`--output`是唯一业务evidence channel且必须等于declared root+result filename；unittest、launcher与test case均不得各自覆盖同一路径。最终validator必须分别验证env/result两份artifact的`tested_commit/tree`、root kind/relative names、schema、canonical command/cwd与artifact digest、profile/result binding与5/5 outcome，任一缺失、双写、root替换/escape、错误digest、运行中或运行后env mutation均fail。
...
Result evidence `gate-results/controlled-real-mcp-result.v1.json` 必须按 11.1 canonicalization 记录 `tested_commit/tree`、启动前冻结且两次验证的env artifact root kind+relative name+digest、typed canonical command/cwd digest、`relevant_source_digest`、protocol/version/adapter、Server ref digest、5个场景ID、脱敏Task/Call ref digest、本地arguments SHA与Server final-input digest关联、每阶段request/effect/commit counter before/after、transport-send与按side-effect/operation分类的restart delta、projection/artifact schema+digest、setup/business/cleanup timeout、TTL/skew、redaction scan与cleanup/revoke result，禁止raw argv/runtime绝对root。`gate-results/controlled-real-mcp-env.v1.json`只承担immutable pre-spawn launcher/profile evidence；最终gate artifact set必须保存、校验并上传两者。
...
| EDGE-39 | pre-spawn artifact 使用错误 checkout/runner-temp root、symlink或escape | 在spawn前拒绝，child/外部副作用0；evidence只存root kind+relative name+digest | environment launcher root fixtures |
| EDGE-40 | Gate 从错误caller cwd运行或actual argv含expanded temp root | launcher使用profile relative cwd；actual argv先投影typed canonical vector并脱敏，mismatch在spawn前失败 | cwd/canonical-command fixtures |
...
| R-31 | 通用pre-spawn artifact root/cwd与CI `RUNNER_TEMP`/checkout布局冲突或泄漏raw argv/绝对路径 | profile-bound root+cwd、typed canonical argv、private0700 runner dir、dirfd/no-follow、sentinel/root-swap fixtures | active |
| R-32 | Frontend/fuzz从repo root或caller cwd运行导致找错package/manifest并向clone外写 | Frontend cwd=`frontend`、fuzz subcommand cwd=`native/fuzz`、allowed-output inventory与arbitrary-caller fixtures | active |
