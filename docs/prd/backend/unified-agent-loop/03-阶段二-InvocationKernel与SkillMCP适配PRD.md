...
- 先压缩history；若稳定规则、完整catalog、当前用户输入和最小suffix仍不适配，采样前fatal
  `agent_tool_catalog_too_large`；
...
| AL-P2-04 | 完整catalog预算超限在采样前失败。 | 恰好容纳、超一项、热重载后超限及no-schema-leak tests。 |
...
- public catalog超预算：采样前fatal，不裁剪Tool；
...
交付Phase 3：`CapabilityInvocationService`、Catalog/Policy、Delegated activation、executable Skill适配和Run-bound MCP
binding。Phase 3不得依赖WorkflowNodePlan或具体executor内部对象。
