...
| 锚点 | 当前事实 | 本阶段约束 |
| --- | --- | --- |
| `src/capabilities/main_agent/executor.py` | 当前存在 `_resolve_skill_matches()` 与 `_run_auto_scripts()` 隐式执行路径。 | 新格式 Skill 命中后只能回答用法或发出 execute/replan signal，不能在 main_agent 内执行 entrypoint。 |
| `src/orchestration/soft_skill_replanner.py` | slash soft binding 已能将 execute signal replan 到 `skill.*`。 | 新格式 slash 与自然语言都要复用显式 `skill.*` 节点路径。 |
...
| C4-009 | 明确区分自然语言规划与隐式 auto-run。 | 普通自然语言可以规划 `skill.*` node；但规划前后都不能在 `main_agent.respond` 内部执行脚本。 |
...
- legacy Skill 行为保持兼容。
- 新格式 Skill 的 answer/execute/finalizer 三分支均有负向测试，确认没有内部脚本执行。
