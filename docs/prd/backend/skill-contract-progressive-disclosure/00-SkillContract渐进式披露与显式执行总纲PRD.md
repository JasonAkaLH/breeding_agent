...
| 证据 | 当前行为 |
| --- | --- |
| `.codex/skills/breeding-skill-builder/references/Skill构建指南.md` | 当前把 `capability_id`、`public_usage`、`parameters`、`scripts`、`execution` 等都列为 `SKILL.md` frontmatter 字段。 |
...
| 用户自定义旧 Skill 不再注册 | 07 在 `.codex/skills/breeding-skill-builder/references/Skill构建指南.md` 和 API 文档明确 v2-only 规则，并提供 v2 模板。 |
