...
| 锚点 | 当前事实 | 本阶段约束 |
| --- | --- | --- |
| `Skill构建指南.md` | 当前仍描述大 frontmatter manifest。 | 指南必须改成轻量 `SKILL.md` + contract + schemas + references 的 v2-only 结构。 |
...
| ID | Requirement | 验收 |
| --- | --- | --- |
| C7-001 | `Skill构建指南.md` 只推荐 v2 结构。 | frontmatter 示例只含 name/description；平台契约全部在 `skill.contract.yaml`。 |
