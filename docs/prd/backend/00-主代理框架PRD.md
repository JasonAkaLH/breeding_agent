...
- 未来 Skill 文件需求必须由 contract/schema 的 `file_intent` / `file_selection` 或 `type: file/artifact/data` 等机器可读字段驱动，平台不得硬编码当前 Skill 名称；实施时必须同步更新 `.codex/skills/breeding-skill-builder/`。
...
- Skill-owned Rust runtime 必须放在各自 Skill bundle 内部，并按 `.codex/skills/breeding-skill-builder/` 的 Rust 型 Skill runtime 限制适配框架 contract；框架不反向兼容某个 Skill 的任意 Rust 形态。
