...
- RUST-FR-066：Skill manifest 中的 Rust metadata 必须遵循 `git@gitee.com:biobin/breeding-skill-builder.git` 的 `references/Skill构建指南.md` 的 `x_runtime.rust` 约定；adapter、contract_version 或构建产物未被 runtime allowlist 支持时必须 fail closed。
