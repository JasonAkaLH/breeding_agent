...
> 来源：从 `docs/prd/backend/00-主代理框架PRD.md` 拆分而来，承载协作共享、结构化 mailbox、interrupt/resume、取消机制与观测事件设计。
...
### 9.3.5 MySQL 只读账号前提
当前 数据查询 Skill 链路关于数据库账号的正式约束为：
- 当前仅配置一个 MySQL 只读账号，但账号、密码、地址与库名只保存在本地 `config.yaml` 或部署密钥环境中
- 该账号必须由数据库侧确认具备只读权限
