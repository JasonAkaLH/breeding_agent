...
6. **可灰度回滚**：selector 从 shadow 到 enforce 分阶段开启；关闭 enforce 后必须回到当前 conversation file context 默认可用路径。

## 拆分原则

1. 父兼容入口保留合并后的完整语义与历史背景；本目录负责把实施拆成可独立计划、开发、验收、回滚的阶段 PRD。
2. 阶段 PRD 默认继承总纲的不变量、禁止字段、安全边界和验收口径，只记录本阶段新增或收窄的范围。
3. Schema / repository / public history 基线先于上传强一致；历史和 prompt 安全先于 selector；selector shadow 先于 enforce；发布门禁最后收口。
...
阶段零 数据模型 / repository / allowlist
  ...
  -> 阶段四 selector enforce / interrupt / attachment provenance
