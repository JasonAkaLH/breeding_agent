...
- **所属专题**：大语言模型提示词信封
- **范围**：`LLMMessage`、`SharedLLMRuntime` messages 入参、`LLMClient` OpenAI-compatible messages 调用、四角色合同、role fallback、thinking/streaming 兼容
- **非范围**：不默认启用 messages 模式；不接入 provider-specific cache hint；不移除 string fallback

## 1. 问题陈述

前几个阶段仍将 PromptEnvelope 渲染为单字符串，以降低迁移风险。但长期目标是 messages-native，让 provider-visible的`system/assistant/user/tool`与内部`context`分类边界更明确，并为provider prompt cache和role-aware安全策略做准备。阶段六要扩展runtime/client能力，同时保证内部context或provider不支持的四角色有deterministic fallback；系统不使用developer角色。
...
5. provider role capability 必须来自启动期 config 或测试显式 config；配置只能声明四角色子集，未知 provider 默认按父计划 deterministic fallback 写入 audit。
...
| P6-FR-2 | role fallback 必须 deterministic。 | active note直接使用system；`tool`可渲染为context block；未知/不支持provider默认fallback；fallback进入audit。 |
...
6. 补齐fake provider tests：四角色闭合、不支持tool、stream thinking、messages preflight。
...
| 风险 | 缓解 |
| --- | --- |
| provider role 支持能力不明确。 | 固定四角色allowlist并默认保守fallback；配置不得声明第五种角色。 |
