...
| `docs/prd/backend/21-文件上传历史消息PRD.md` | 上传成功即写入结构化 `file_upload` 历史消息，约束 system message allowlist、deleted 文件不可复用与 `index.md` 强一致 | 设计已确认，待实施 |
| `docs/prd/backend/22-聊天式会话文件智能选择PRD.md` | 基于用户 query、Skill 文件需求契约、文件元数据和 recent usage 的聊天式文件自动定位与 interrupt 消歧 | 设计已确认，待实施 |
| `docs/prd/backend/23-Skill运行闭环Workbench总纲PRD.md` | 平台层 Skill 运行闭环 Workbench、内部 `workbench.*` capability、执行后验证与受控重编排总纲 | 总体设计稿，待拆分阶段实施 PRD |
