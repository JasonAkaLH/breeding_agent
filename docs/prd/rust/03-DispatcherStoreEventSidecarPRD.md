...
- **状态**：部分落地（`native/proto/maf/runtime/v1/`、`maf_runtime_store` / `maf_event_log` / `maf_task_dispatcher` contract/kernel 基线已落地，`maf_runtime_sidecar` service kernel 已开始承载 sidecar 写语义、health/readiness/drain 生命周期、RPC-shaped response envelope、tonic/prost gRPC binding 与 `maf-runtime-sidecar` 进程入口；`RuntimeSidecarSqliteAdapter` durable SQLite event cursor / idempotency / lease / task / node / task edge / artifact metadata / cancellation / bundle revision 持久化已落地；Python event append/replay facade 已消费 Rust resource/page/deadline/backpressure limit，且 `SQLiteStorage(runtime_sidecar_client=...)` 在 enforce 模式下可将 task submit、node transition、event append、task edge save 与 artifact metadata save 路由到已配置 sidecar client 并禁止 Python SQLite legacy 写入，在 shadow 模式下已可对 task submit、node transition、event append、task edge save、artifact metadata save、cancellation token write 与 Skill/MCP bundle revision pin/release 执行 sidecar 旁路对比并记录脱敏 audit；`RuntimeSidecarGrpcClient` 已可通过内部 h2c gRPC loopback、mTLS TCP 或 Unix domain socket 连接外部 Rust sidecar binary 并覆盖 version / compatibility、task/node/event、task edge、artifact metadata、lease、cancellation token、bundle revision RPC；promotion、migration/DR、ops readiness 与 decommission evidence gates 已落地；GitHub Actions 已补 RuntimeSidecar Linux x86_64 binary 的 SBOM / provenance / manifest 上传口径，`docs/prd/rust/evidence/prd03/runtime_sidecar_release_gates.json` 与 `scripts/validate_prd03_runtime_sidecar_evidence.py` 负责 release-gate 证据账本 fail-closed 校验；PRD03 剩余未完成项收敛为生产 enforce rollout / 7 天 production shadow promotion 证据、ops / migration / rollback drill 实际执行证据、部署 allowlist promotion 证据与 Python legacy 写路径最终下线）
- **日期**：2026-05-14
...
- RUST-SIDE-FR-012：必须 fail-closed 的写入类操作包括 task submit / create、node state transition、task edge save、artifact metadata save、event append、lease acquire / renew / release、cancellation token 写入、bundle revision pin / release。
...
- RUST-SIDE-FR-026：runtime sidecar 必须建立 task submit、state transition、task edge save、artifact metadata save、event append、lease、event replay、SSE snapshot 的 Python baseline 与 Rust sidecar benchmark；P95 / P99、queue wait、CPU、memory、replay throughput 必须纳入 release gate。
- RUST-SIDE-FR-027：SQLite schema、event log、lease、cursor、task graph edge、artifact metadata、bundle pin 与 future PostgreSQL-compatible contract 的任何状态变更必须具备 schema version、migration lock、preflight、dry-run、backup、restore、event replay 校验与 rollback / roll-forward runbook。
...
| Task edge | `task_id`、`from_node_id`、`to_node_id`、`edge_type`、`condition` |
| Artifact metadata | `artifact_id`、`task_id`、`producer_node_id`、`artifact_type`、`storage_ref`、`summary`、`is_complete`、`created_at` |
...
2. Performance SLO：task submit、state transition、task edge save、artifact metadata save、event append、lease、event replay 与 SSE snapshot 必须有 Python baseline、Rust sidecar baseline、P50/P95/P99、queue wait、CPU、memory、throughput 指标；性能回归不得进入 `enforce`。
3. 迁移 / 容灾：SQLite schema、event log、lease、cursor、task graph edge、artifact metadata、bundle pin 的 migration 必须执行 migration lock、preflight、dry-run、backup、restore、replay 校验；失败时不得接受新写入。
...
Enforce 失败处理策略冻结：`enforce` 模式下 Rust kernel / sidecar 失败默认 fail closed。对 Dispatcher / Store / Event sidecar，本 PRD 明确禁止 task submit / create、node state transition、task edge save、artifact metadata save、event append、lease acquire / renew / release、cancellation token 写入、bundle revision pin / release 等状态写入类操作自动 fallback 到 Python legacy store；只允许无状态或无副作用 read-only 查询按本文档受限降级。所有失败必须写 structured audit，并返回稳定 typed error。写路径可对同一个 Rust sidecar 做幂等自动重试，但必须由 `retriable=true`、idempotency key 与 retry policy 驱动。
...
- `SQLiteStateRepository.save_task_edge()` 与 `save_artifact()` 也纳入 runtime store enforce guard：task graph edge 消费 Rust `task_edge_save` policy，artifact metadata 消费 Rust `artifact_save` policy；`MAF_RUST_RUNTIME_STORE_MODE=enforce` 且当前尚未注入 Rust sidecar client 时，edge / artifact metadata 写入以 `runtime_store_unavailable` fail-closed。
...
- `validate_runtime_sidecar_response()` 按 Rust sidecar contract 校验正式 client 将消费的 structured response envelope，包括 operation、event cursor、lease、task / node / task edge / artifact metadata / cancellation / bundle response 与 typed error code / category / retriable / safe metadata；校验失败以 `runtime_store_response_invalid` fail-closed，不推进 cursor、不提交状态、不释放 lease。
...
- Runtime sidecar contract artifact 补齐 max-in-flight 公式参数与 task submit / state transition / task edge save / artifact metadata save / event append / lease / event replay deadline hard caps；`runtime_sidecar_max_in_flight()` 只按 Rust artifact 计算 `min(64, cpu * 4)` 且最低 8，避免 Python 自带生产并发常量。
...
- Runtime sidecar contract artifact 新增 benchmark policy 与 `runtime_store_benchmark_invalid` typed error；`validate_runtime_sidecar_benchmark_report()` 要求 promotion 前同时提供 Python baseline 与 Rust sidecar baseline，并覆盖 task submit、node transition、task edge save、artifact metadata save、event append、lease、event replay、SSE snapshot 的 P50/P95/P99、queue wait、CPU、memory 与 throughput 指标。
...
- `native/crates/maf_runtime_sidecar` 新增 transport-independent Rust service kernel，组合 `maf_task_dispatcher`、`maf_event_log` 与 `maf_runtime_store::LeaseRegistry`，先由 Rust 持有 version / compatibility、task submit、node transition、task edge、artifact metadata、event append/replay、lease、cancellation token 与 bundle revision pin/release 状态；后续 tonic/gRPC wrapper 与 SQLite adapter 必须委托该 kernel，不得在 Python client/facade 重新实现写语义。
...
- `maf_runtime_sidecar` 新增 protobuf/RPC-shaped service adapter 与 `TypedErrorEnvelope`，先用 Rust request / response DTO 对齐 runtime proto 的 health/readiness、compatibility、append/replay、task edge、artifact metadata、lease、cancellation token、bundle revision 等 response envelope；正式 tonic transport 接入时只应绑定该 adapter，不得在 Python facade 中重新拼装 typed error 或 cursor response。
- `maf_runtime_sidecar` 已通过 `tonic` / `prost` / `tonic-prost-build` 从 `native/proto/maf/runtime/v1/runtime.proto` 生成 Rust gRPC binding，并新增 `RuntimeSidecarGrpcService` 将 protobuf request 映射到 Rust adapter、将 Rust typed error envelope 映射回 protobuf `TypedError`；`RuntimeSidecarGrpcService::with_sqlite_adapter()` 与 `RuntimeSidecarServeConfig::build_service()` 已可把 append/replay 等 RPC 委托给 durable SQLite adapter 并通过 reopen 回放验证，`maf-runtime-sidecar --serve 127.0.0.1:<port> --sqlite <path>`、`maf-runtime-sidecar --serve unix:///absolute/path --sqlite <path>` 与 `maf-runtime-sidecar --serve <addr> --sqlite <path> --tls-cert <cert> --tls-key <key> --client-ca <ca>` 均可由外部进程配置 SQLite adapter；server-side 环境变量 `MAF_RUNTIME_SIDECAR_TLS_CERT_PATH` / `MAF_RUNTIME_SIDECAR_TLS_KEY_PATH` / `MAF_RUNTIME_SIDECAR_TLS_CLIENT_CA_PATH` 也可启用 mTLS，public TCP bind 只有在完整 server certificate / key / client CA mTLS 配置存在时才允许。
- `maf_runtime_sidecar` 新增 `maf-runtime-sidecar` 二进制入口，提供 `--version` 与 `--serve <addr>`，进程入口直接承载 Rust `RuntimeSidecarGrpcService`；Rust `RuntimeSidecarServeConfig` 支持 `unix:///absolute/path` 内部 Unix socket endpoint，非 loopback TCP 监听地址必须通过完整 mTLS 配置显式启用；该入口为外部进程管理器 / 容器编排启动 sidecar 准备，Python 生产请求路径仍不得负责 spawn / restart / kill sidecar。
- `maf_runtime_sidecar` 新增 `RuntimeSidecarSqliteAdapter` durable SQLite adapter，覆盖 event append/replay cursor、event idempotency、task lease acquire/renew/release、task submit、node transition、task edge、artifact metadata、cancellation token 与 bundle revision pin/release 的 reopen 后持久化，并保持重复 idempotency key 返回原始结果。
- `SQLiteStorage` 新增 `runtime_sidecar_client` 注入路径；当 runtime_store / event_log 处于 `enforce` 且 client 已配置时，task submit、node transition、task edge save、artifact metadata save 与 event append 先调用 sidecar client 并通过 `validate_runtime_sidecar_response()` 校验 response envelope，成功后不写 Python SQLite legacy 表；edge / artifact read 在配置 client 的 enforce 模式下也优先从 sidecar 读取，避免读到 Python legacy 表的陈旧状态；未配置 client 时写路径仍按 typed error fail-closed。
- `SQLiteStorage` 新增 `runtime_sidecar_shadow_sink` 注入路径，`src/storage/runtime_sidecar_shadow.py` 提供 RuntimeSidecar shadow compare / audit 共用 helper；当 runtime_store / event_log / task_dispatcher 处于 `shadow` 且已配置 sidecar client 与 audit sink 时，task submit、node transition、event append、task edge save、artifact metadata save、cancellation token write 与 Skill/MCP bundle revision pin/release 仍以 Python legacy 写入作为用户可见结果，同时旁路调用 sidecar、校验 response envelope，并将 component、operation、input / legacy / rust fingerprint、error code 与 duration 写入 `runtime.sidecar_shadow_diff` 脱敏 audit；sidecar 或 audit 失败不得影响用户可见结果，事件 payload 与 artifact `storage_ref` / summary 等敏感字段只记录 hash，不记录完整 payload / secret / path。
- `RuntimeSidecarGrpcClient` 新增无需额外 Python 依赖的内部 h2c / mTLS gRPC unary client，连接前校验 endpoint allowlist 与 config source，运行时在每个 operation 前执行 compatibility handshake，并按 Rust response envelope 校验 version / compatibility、task submit、node transition、task edge save/list、artifact metadata save/get/list、event append/replay、lease acquire/renew/release、cancellation token write、bundle revision pin/release；`tests/integrations/test_runtime_sidecar_grpc_client.py` 会拉起外部 `maf-runtime-sidecar --serve 127.0.0.1:<port> --sqlite <path>`、`maf-runtime-sidecar --serve unix:///absolute/path --sqlite <path>` 与 `maf-runtime-sidecar --serve 127.0.0.1:<port> --sqlite <path> --tls-cert <cert> --tls-key <key> --client-ca <ca>` 做真实 RPC 验证。
- `build_api_runtime()` 可通过 `MAF_RUNTIME_SIDECAR_ENDPOINT`（可选 `MAF_RUNTIME_SIDECAR_ALLOWED_HOSTS` / `MAF_RUNTIME_SIDECAR_MTLS_ENABLED` / `MAF_RUNTIME_SIDECAR_TLS_CA_PATH` / `MAF_RUNTIME_SIDECAR_TLS_CERT_PATH` / `MAF_RUNTIME_SIDECAR_TLS_KEY_PATH` / `MAF_RUNTIME_SIDECAR_TLS_SERVER_NAME`）装配 `RuntimeSidecarGrpcClient`；在已配置 sidecar client 的 enforce 模式下，lifecycle cancellation token、RuntimeLeaseFacade 与 ApiRuntime bundle revision pin/release 也会路由到 sidecar response envelope，未配置 client 时继续 fail-closed。
- 当前实现仍是 contract/kernel + Python facade / client 消费基线；edge / artifact metadata 独立 RuntimeSidecar coverage 已落地，RuntimeSidecar binary 的 CI SBOM / provenance / manifest 生成与 evidence ledger fail-closed 校验也已接入。仍未完成的是 production enforce rollout、7 天 production shadow promotion 数据、ops / migration / rollback drill 实际执行记录、部署 allowlist promotion 以及 Python legacy 写路径最终下线；这些不能由本地样例或合成证据替代。

## 11. 测试策略

| 层级 | 测试 |
|---|---|
| Rust unit | lease、idempotency、event append/replay、cursor ordering、version / compatibility、health/readiness/drain、RPC-shaped response envelope、tonic/protobuf binding、sidecar binary smoke、task edge、artifact metadata、cancellation token、bundle pin/release |
| Integration | externally managed sidecar start/health/shutdown、loopback / Unix socket / mTLS transport、dev launcher、client protocol compatibility、rolling upgrade matrix、structured output validation、task edge / artifact metadata real RPC、endpoint allowlist validation、RuntimeSidecar binary artifact checksum / SBOM / provenance / manifest validation、release-gate evidence ledger fail-closed validation |
| Python regression | `tests/storage`、`tests/api`、`tests/e2e`；当前覆盖 Rust contract accessors、sidecar handshake compatibility、sidecar endpoint allowlist fail-closed、structured response envelope fail-closed、same-sidecar idempotent retry gate、max-in-flight / deadline hard caps、config source / cross-host mTLS identity gate、artifact checksum / provenance gate、benchmark report completeness gate、shadow → enforce promotion threshold gate、migration / DR evidence gate、ops readiness evidence gate、decommission readiness evidence gate、事件 payload limit fail-closed、event replay page limit fail-closed、event log enforce no-legacy-fallback、runtime store task/node/task edge/artifact metadata enforce no-legacy-fallback 与 configured-client routing、edge / artifact metadata enforce read path、cancellation token enforce no-legacy-fallback、bundle revision pin/release enforce no-legacy-fallback、lease acquire/renew/release enforce no-legacy-fallback、task/node/event/task edge/artifact metadata/cancellation token/bundle revision shadow audit、分页 replay facade、跨页 API/SSE 历史回放与正常事件往返 |
...
| Performance | task submit + event replay smoke、task edge / artifact metadata write smoke、large event stream、Python baseline vs Rust sidecar P50/P95/P99 / throughput / CPU / memory；正式 enforce 前必须把真实报告写入 PRD03 evidence ledger 并通过严格模式校验 |
| Migration / DR | schema migration lock、backup、restore、event replay、task edge / artifact metadata reopen verification、rollback / roll-forward drill |
...
| RUST-SIDE-AC-013 | runtime sidecar benchmark 覆盖 task submit、state transition、task edge save、artifact metadata save、event append、lease、event replay、SSE snapshot 与资源指标 | benchmark report + CI / release SLO gate |
| RUST-SIDE-AC-014 | SQLite schema / event log / cursor / lease / task graph edge / artifact metadata / bundle pin migration 有 backup、restore、replay 与 rollback / roll-forward 演练 | migration tests + restore / replay drill |
