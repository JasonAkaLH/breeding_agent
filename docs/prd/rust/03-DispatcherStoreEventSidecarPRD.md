...
- 当前实现仍是 contract/kernel + Python facade / client 消费基线；edge / artifact metadata 独立 RuntimeSidecar coverage 已落地，但 production enforce rollout、7 天 shadow promotion 证据、ops / migration / rollback drill、artifact provenance allowlist 远端证据与 Python legacy 写路径最终下线仍未完成。
...
| Integration | externally managed sidecar start/health/shutdown、loopback / Unix socket / mTLS transport、dev launcher、client protocol compatibility、rolling upgrade matrix、structured output validation、task edge / artifact metadata real RPC、endpoint allowlist validation、artifact checksum / provenance validation |
...
| Performance | task submit + event replay smoke、task edge / artifact metadata write smoke、large event stream、Python baseline vs Rust sidecar P50/P95/P99 / throughput / CPU / memory |
