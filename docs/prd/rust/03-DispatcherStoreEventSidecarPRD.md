...
- `SQLiteStorage` 新增 `runtime_sidecar_shadow_sink` 注入路径，`src/storage/runtime_sidecar_shadow.py` 提供 RuntimeSidecar shadow compare / audit 共用 helper；当 runtime_store / event_log / task_dispatcher 处于 `shadow` 且已配置 sidecar client 与 audit sink 时，task submit、node transition、event append、cancellation token write 与 Skill/MCP bundle revision pin/release 仍以 Python legacy 写入作为用户可见结果，同时旁路调用 sidecar、校验 response envelope，并将 component、operation、input / legacy / rust fingerprint、error code 与 duration 写入 `runtime.sidecar_shadow_diff` 脱敏 audit；sidecar 或 audit 失败不得影响用户可见结果，事件 payload 只记录 hash，不记录完整 payload / secret。edge / artifact 当前没有独立 sidecar RPC，仍只保留 enforce guard 与后续覆盖口径决策。
...
- 当前实现仍是 contract/kernel + Python facade / client 消费基线，不代表 edge / artifact shadow 覆盖口径、enforce rollout、artifact provenance allowlist 或 Python 写路径最终下线已经完成。
