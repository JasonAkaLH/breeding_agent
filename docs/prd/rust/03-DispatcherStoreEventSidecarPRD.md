...
- `SQLiteStorage` 新增 `runtime_sidecar_shadow_sink` 注入路径；当 runtime_store / event_log 处于 `shadow` 且已配置 sidecar client 与 audit sink 时，task submit、node transition 与 event append 仍以 Python SQLite legacy 写入作为用户可见结果，同时旁路调用 sidecar、校验 response envelope，并将 component、operation、input / legacy / rust fingerprint、error code 与 duration 写入 `runtime.sidecar_shadow_diff` 脱敏 audit；sidecar 或 audit 失败不得影响用户可见结果，事件 payload 只记录 hash，不记录完整 payload / secret。
...
- 当前实现仍是 contract/kernel + Python facade / client 消费基线，不代表 shadow 覆盖面已完整、enforce promotion、artifact provenance allowlist 或 Python 写路径最终下线已经完成。
