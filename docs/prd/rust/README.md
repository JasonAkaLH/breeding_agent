...
| 07 | `07-OrchestrationDeterministicKernel与热点优化PRD.md` | orchestration deterministic kernel 与热点小 kernel；条件候选专题 | 条件候选 guard 已落地：`docs/prd/rust/evidence/prd07/orchestration_hotspot_release_gates.json` + `scripts/validate_prd07_orchestration_hotspot_evidence.py`，不启动 Rust 化 |
...
- 做 PRD07 orchestration / hotspot 相关 Rust 或 WASM 实现前，必须先通过 `scripts/validate_prd07_orchestration_hotspot_evidence.py --json` 确认当前仍是 guarded 状态；若要启动，需另开实施 PRD 并把 evidence ledger 升级到 `candidate_ready_to_start` 且严格校验通过。
