...
| 01 | `01-Rust工具链构建发布与质量门禁PRD.md` | rustup / Cargo / PyO3 / sidecar 构建、CI、质量门禁 | Rust workspace 多 crate contract/kernel 基线、PRD01 Rust quality workflow、本地 gate runner、PyO3 wheel smoke、Ubuntu 22.04 x86_64 `manylinux_2_35` wheel CI 目标、artifact provenance self-check 与 fuzz harness compile smoke 已落地；真实 CI 运行结果、coverage 阈值 / 长时 fuzz / 真实 SBOM-provenance 产物门禁待完成 |
