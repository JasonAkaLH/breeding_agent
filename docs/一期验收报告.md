...
**结论：一期范围内承诺的“主代理最小内核 + SQLQuery 只读 MVP + FastAPI/SSE/cancel/query API”已完成，当前仓库可判定为一期验收通过。**
...
### 3.1 Happy path
- 文件：`tests/e2e/test_sql_query_happy_path.py`
- 证明链路：
  1. 提交消息
  2. 背景执行生成 DAG
  3. guard 通过
...
### 3.2 Guard blocked
- 文件：`tests/e2e/test_sql_query_guard_block.py`
- 证明链路：
  1. 通过 deterministic `sql_generator` 生成危险 SQL
  2. `sql_query.sql_guard_blocked` 写入审计
  3. 任务收敛为 `failed`
