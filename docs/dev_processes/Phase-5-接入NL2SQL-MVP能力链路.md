...
## 输出
- `src/capabilities/nl2sql/` 目录
- 适配 Phase 4 orchestration 标准的 NL2SQL workflow definition / plan provider
- NL2SQL capability 级实现
- MySQL 只读执行适配层
- NL2SQL 专项测试
- 一条 capability 闭环验证结果

## 要做什么
- 将现有 `src/nl2sql/` 资产按 capability 级边界收口。
- 提供符合 Phase 4 通用 workflow / task plan 标准的 NL2SQL workflow definition。
- 实现以下能力单元：
  - `intent_route`
...
## 缓解建议
- NL2SQL 必须适配 Phase 4 的 orchestration 标准，不应要求主代理为其增加专用业务分支。
- capability 内部实现必须完全隐藏在 `src/capabilities/nl2sql/` 与 `src/integrations/` 后面。
