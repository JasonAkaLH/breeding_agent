...
- `superpowers/specs/2026-08-18-mcp-dispatch-aggregate-recovery-hardening-design.md`：MCP dispatch 聚合状态机与恢复加固设计；在保持 v2 引用式信封和 no-replay 边界的前提下，统一 Tool approval、普通多次调用、MRTR、remote Task、terminal candidate、claim 与 startup recovery 的 SQL 原子状态合同，并定义分阶段实施和故障注入验收。
...
| `superpowers/specs/2026-08-18-mcp-dispatch-aggregate-recovery-hardening-design.md` | 设计已确认；尚未实施 | 按 Phase 0～4 先修复 CP7 时间与 post-admission 收敛，再统一聚合事务、持久化审批 action、MRTR/remote Task恢复、claim/预算/candidate GC，并完成12个崩溃边界的SQLite/PostgreSQL故障注入。 |
