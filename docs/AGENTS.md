...
- `superpowers/specs/2026-08-22-unified-agent-loop-implementation-plan.md`：依据已批准PRD组生成、经三轮自主document-perfectization以99/100通过的统一同模型Agent Loop详细实施计划；Phase 0～Phase 6已闭合，P6 cutover bundle与删除报告已建立，下一检查点P7-A备份/restore-all演练。
...
- `prd/backend/unified-agent-loop/`：当前任务执行/恢复唯一PRD authority；Phase 0～Phase 5 `proof_complete`，Phase 6 `cutover_complete`，旧runtime/wiring已删除，下一检查点P7-A。
...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `prd/backend/unified-agent-loop/README.md`（来源：统一Agent Loop架构与PRD decomposition design） | Phase 6 `cutover_complete` | 执行P7-A三backend仓库外备份与restore-all演练；P7-A通过前不得进入破坏性schema/proto删除。 |
