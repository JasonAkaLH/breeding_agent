...
- `superpowers/specs/2026-08-20-mcp-result-text-artifact-design.md`：MCP Tool JSON原始返回改为公共`text` artifact的增量设计；内部继续复用受校验文件副本与生命周期，任务/历史API内联原始UTF-8正文且不提供下载，前端以可展开原始返回卡片展示，新旧MCP产物统一生效。
