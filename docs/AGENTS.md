...
- `superpowers/specs/2026-08-21-unified-agent-loop-prd-decomposition-design.md`：已确认、待用户复核书面版本的PRD拆分设计；采用README、总纲与Phase 0～7八份阶段PRD，按Model Contract、Agent Storage/Lease、Invocation/Skill/MCP、核心Loop、Continuation、API/Frontend、Clean Cutover、破坏性Schema与最终证明的严格依赖推进，并为FR-1～FR-26指定唯一主责阶段。
...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `superpowers/specs/2026-08-21-unified-agent-loop-design.md`及PRD decomposition design | 架构与8阶段拆分均已确认；实现尚未开始 | 用户复核书面拆分设计后，生成`docs/prd/backend/unified-agent-loop/`的README、总纲与Phase 0～7阶段PRD；整组PRD获批后再创建实施计划，不提前修改业务代码。 |
