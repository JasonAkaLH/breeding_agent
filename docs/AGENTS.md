...
- `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-design.md`：已确认、尚未实施的MCP Tool原始返回Artifact设计；成功且具有authoritative durable result的每个Call通过现有promotion桥接进入与Skill相同的公共Artifact/API/下载/前端卡片标准，失败或取消Call只展示脱敏错误码，并覆盖approval、remote、startup补偿与未过期历史结果补投。
...
| `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-design.md` | 设计已确认；尚未实施 | 复用现有MCP durable-result promotion与Skill公共Artifact交付链，实现每个成功Call一个原始JSON Artifact、无网络重放补偿、未过期历史补投和真实OCR下载验收。 |
