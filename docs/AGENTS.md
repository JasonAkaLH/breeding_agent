...
- `superpowers/specs/2026-08-24-project-wide-progressive-architecture-cleanup-design.md`：已逐节获批并完成scope reset的全仓业务代码渐进式架构清理总设计；删除前九轮发散出的通用gate/evidence与真实MCP控制平台设计，重新聚焦全业务源码inventory、稳定facade、唯一owner、不可变业务时序、P0～P8小检查点、条件性平台验证和独立回滚；具体文件/测试/CI/artifact细节下沉到各计划基于最新HEAD决定，现待精简版独立信心门复审，不混入行为修复、schema/data migration或`prod`变更。
