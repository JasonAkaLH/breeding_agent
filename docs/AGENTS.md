...
- `superpowers/specs/2026-08-18-mcp-dispatch-aggregate-recovery-hardening-design.md`：通过95%信心门的 MCP dispatch 聚合状态机与恢复加固设计；保持v2引用式信封和no-replay，统一Tool approval、普通多Call、MRTR、remote Task、durable result、terminal candidate、claim、取消线性化与startup recovery的SQL原子状态合同，并定义分阶段实施和18项FR/17个崩溃边界验收。
- `superpowers/specs/2026-08-18-mcp-dispatch-aggregate-recovery-hardening-implementation-plan.md`：以96%置信度通过95%信心门的开发计划；按5个Phase拆分green checkpoint，Phase 1～3禁止启动和旧新writer混跑，覆盖受控SQLite/PostgreSQL cutover、pending action、MRTR/remote恢复、candidate/result lifecycle、18项FR/8项NFR和17个故障注入边界。
...
| `superpowers/specs/2026-08-18-mcp-dispatch-aggregate-recovery-hardening-design.md`及对应implementation plan | 设计96%、计划96%通过95%文档信心门；代码尚未实施 | 严格按implementation plan的green checkpoint执行；Phase 1～3连续完成且migration report/apply门禁通过后才允许重启本地backend，最终完成17个崩溃边界和真实PostgreSQL证据。 |
