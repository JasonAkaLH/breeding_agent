...
- `superpowers/specs/2026-08-24-project-wide-progressive-architecture-cleanup-design.md`：已逐节获批并进入document-perfectization循环的全仓业务代码渐进式架构清理总设计；第7轮将pre-spawn evidence root闭合为profile-bound checkout/runner-temp安全合同，Result Parser使用CI私有0700 root，fuzz/真实MCP使用checkout并安全替换tracked evidence；其余terminal、PG freshness、三backend、Frontend与公开合同保持闭合，现待第8轮独立复审；不混入行为修复、schema/data migration或`prod`变更。
