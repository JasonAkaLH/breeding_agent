...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p2-implementation-plan.md`：基于P1 handoff与`92a8a60` HEAD启动的P2 Orchestration/Capabilities边界计划；只让四个P2-owned persistence consumer采用窄port、复用capability-gap prompt formatter，并以P0 exact trace复验continuation/Interrupt/MCP seams；不拆高风险C901、不迁移P3～P5实现或修deferred behavior。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p2-implementation-plan.md` | P2实施`active`；4个aggregate/untyped consumer、1处Prompt文本复制及高风险no-change项已审计 | 按A～D迁移P2-owned annotation、去除单一Prompt复制并运行Backend全量；不进入P3。 |
