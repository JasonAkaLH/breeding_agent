...
- `prd/backend/unified-agent-loop/`：依据上述100/100拆分设计生成的统一同模型Agent Loop PRD组；README维护状态，总纲维护FR/NFR与全局门禁，Phase 0～7依次覆盖Model Contract、Agent Storage/Lease、Invocation/Skill/MCP、核心Loop/final、Continuation、API/Frontend/Observability、Clean Cutover与DAG runtime删除、破坏性Schema删除和最终证明；整组待用户复核，尚未实施。
...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `prd/backend/unified-agent-loop/README.md`（来源：统一Agent Loop架构与PRD decomposition design） | README、总纲与Phase 0～7阶段PRD已生成；整组待复核，实现尚未开始 | 复核整组PRD；获批后创建详细实施计划，不提前修改业务代码。 |
