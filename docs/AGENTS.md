...
- `superpowers/specs/2026-08-24-project-wide-progressive-architecture-cleanup-design.md`：已逐节获批并进入document-perfectization循环的全仓业务代码渐进式架构清理总设计；第3轮已闭合路径终态/dirty隔离gate、Agent双阶段Node与resume/parallel异常、Gateway mixed-lock/双barrier、Parser post-checkpoint cancel、Frontend跨Hook补偿、Storage公开wrapper/Collaboration session、Rust PyO3/field owner/public attrs/稳定Ubuntu leaf，并定义versioned真实MCP control协议与Result Parser Linux CI job，现待第4轮独立复审；不混入行为修复、schema/data migration或`prod`变更。
