...
| `prd/MCP/user-scoped-on-demand/` | 阶段一已实现；阶段二的两级路由、授权、调用账本、长调用控制、SSE 租约、前端配置/状态与专题审计已实现，2026 MRTR/Tasks 跨重启恢复仍待完成；阶段三待实施 | 后续补齐 MRTR 密封恢复与标准 Tasks worker，再进行五版本灰度和旧 Runtime 下线；用户工具始终不得注册为全局 Capability。 |
