...
| `superpowers/specs/2026-08-19-ocr-mcp-trusted-attachment-workflow-design.md` | 主仓实现、外部ocr_mcp严格source schema与自动回归已完成；真实smoke进行中 | 重建本地backend并用用户报纸PNG完成approval/OCR终态验收；大于10 MiB companion upload和远端OCR源码部署保持独立后续工作。 |
