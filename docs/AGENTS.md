...
| `prd/MCP/user-scoped-on-demand/` | 用户级按需 MCP 三阶段产品决策已确认；`2026-07-28` 第五协议版本已纳入目标；待实施 | 依次交付用户配置/凭据/五版本 Gateway、两级路由/授权/MRTR/Tasks 闭环、五版本灰度切换/旧 Runtime 下线；不得提前持久化 Tool List 或将用户工具注册为全局 Capability。 |
