...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `prd/backend/unified-agent-loop/README.md`（来源：统一Agent Loop架构与PRD decomposition design） | Phase 0 `proof_complete`；Phase 1 `blocked`，P1-A green、P1-B本地31项通过但真实PG 0 tests/1 skip | 提供非生产`MAF_POSTGRES_TEST_DSN`并零skip通过P1-B后，才进入P1-C；保持Phase 6前当前DAG运行时不变。 |
