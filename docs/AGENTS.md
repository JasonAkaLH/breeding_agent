...
- `superpowers/specs/2026-08-24-project-wide-progressive-architecture-cleanup-design.md`：已逐节获批并进入document-perfectization循环的全仓业务代码渐进式架构清理总设计；第5轮补齐Cancellation enforce无client零写、全API runtime seam、P5 PostgreSQL operation→leaf、Interrupt outcome订阅矩阵、terminal continuation/TaskNode预投影/authority snapshot现状合同、全部ignored AGENTS发现、真实MCP双artifact唯一producer+pre-dedup no-replay+逐连接SPKI、Rust fuzz summary和Scripts canonical-test/三backend evidence，现待第6轮独立复审；不混入行为修复、schema/data migration或`prod`变更。
