...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p8-implementation-plan.md`：基于P7 handoff与`ff0a65d` HEAD启动的P8最终收尾计划；只处理Auth窄port漏项、已证明Python/Frontend/Native dead/duplicate finding，并闭合345个business path与全仓最终门禁；跨authority exact body和行为缺陷不动。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p8-implementation-plan.md` | P8实施`active`；1093 tracked、345 business、dead/duplicate/facade/dependency finding已重审 | 按A～F处置明确finding并运行Backend/Frontend/Rust最终全量；不扩散到行为修复。 |
