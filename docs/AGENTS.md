...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p5-implementation-plan.md`：基于P4 handoff与`af6e3b0` HEAD启动的P5 persistence边界计划；只合并2组pure helper、建立shared SQLAlchemy/model/mapper owner并让Lifecycle采用窄ports；transaction/lock/CAS、PG override、Sidecar与P4 selector不动，终态要求本地PostgreSQL 7个隔离profile零skip。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p5-implementation-plan.md` | P5实施`active`；117项focused baseline、2组exact duplicate、shared declaration/private mapper反向依赖、5个Lifecycle port与29个C901已审计 | 按A～F只做pure helper/shared declarations/mappers/narrow ports；用7个隔离PG数据库验收，不改事务或schema/data。 |
