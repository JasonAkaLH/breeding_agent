...
- `prd/backend/unified-agent-loop/cutover-readiness.md`：Phase 5 closed handoff已增补P6-A预检证据；canonical storage在七个隔离PG库中441项零skip通过，当前仅外部Agent Skill suite required skip未闭合，因此尚未冻结clean rollback archive或进入P6-B。
...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `prd/backend/unified-agent-loop/README.md`（来源：统一Agent Loop架构与PRD decomposition design） | Phase 0～Phase 5 `proof_complete`；P6-A `blocked` | 提供与现行合同匹配的权威外部Skill bundle/runtime，重跑P6-A全门禁后才可冻结clean archive；此前保持DAG runtime不变。 |
