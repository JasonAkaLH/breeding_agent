...
- `superpowers/specs/2026-08-17-dollar-mcp-server-soft-binding-design.md`：`$显示名称` 主动选择用户级 MCP Server，强制当前 Server discovery，由 Selector决定 Tool调用并保留逐 Tool授权；菜单仅含 available Server，绑定一次性生效，附件仅提供安全摘要且不在本轮传输正文。
