...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p7-implementation-plan.md`：基于P6 handoff与`a5c9318` HEAD启动的P7 Native/Scripts边界计划；四workstream独立处置Sidecar exact helper/codec、Skill stdio/service、MCP private boundaries/fuzz合同门和单文件Scripts engine lifecycle；public/Proto/Cargo/PyO3/CLI/SQL/transaction合同不动。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p7-implementation-plan.md` | P7实施`active`；Rust/Scripts基线及四workstream候选已审计 | 按A～G先补MCP合同门，再分workstream迁移private实现；不改公开/数据/行为。 |
