...
- `superpowers/specs/2026-08-24-project-wide-progressive-architecture-cleanup-design.md`：已逐节获批并进入document-perfectization循环的全仓业务代码渐进式架构清理总设计；第4轮已闭合Python/API/HTTP/runtime seam全量合同、closed gate env、API lifecycle故障矩阵、Agent heartbeat旧token与parallel五入口现状、resume resolver次数、Parser cleanup-join cancel、Gateway `on_registered`、Frontend optimistic assistant binding、non-aggregate Cancellation writer/Sidecar persistence、PG operation/SQL owner、Rust field-owner/fuzz/PyO3/Scripts stable gates、P8最终依赖，并将Result Parser Linux改为机器artifact runner、将真实MCP freshness扩到P1至P7完整durable调用链，现待第5轮独立复审；不混入行为修复、schema/data migration或`prod`变更。
