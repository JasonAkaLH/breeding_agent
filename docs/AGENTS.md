...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p4-implementation-plan.md`：基于P3 handoff与`477b467` HEAD启动的P4 API Runtime边界计划；只复用request runtime accessor、分离连续upload职责块并收窄两个private persistence annotation；公开runtime/factory/route、Interrupt/file-selection/lifecycle/backend selector合同不动。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p4-implementation-plan.md` | P4实施`active`；183项focused baseline、0组三语句exact duplicate、7份runtime accessor复制与29个C901已审计 | 按A～E只做request accessor复用、upload mixin与private port annotation；不进入P5 adapter或改Interrupt/lifecycle控制流。 |
