...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p1-implementation-plan.md`：基于P0 handoff与`97d78c6` HEAD启动的P1窄persistence contract计划；把259-method `StoragePort`按现有业务块拆为19个无重叠Protocol，原对象只保留aggregate兼容facade，并以本地单方法Protocol收窄Cancellation Sidecar writer；不迁移P2～P7 consumer、不修deferred behavior或修改schema/data。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p1-implementation-plan.md` | P1实施`active`；P1-A当前HEAD、259-method分区、36个生产引用文件与Cancellation边界审计完成 | 按Checkpoint A～D先补membership RED锁，再拆窄Protocol、收窄Cancellation类型并运行Backend全量；不进入consumer迁移。 |
