...
- `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-design.md`：以98%置信度通过95%信心门、尚未实施的MCP Tool原始返回Artifact设计；每个authoritative completed业务Call经唯一projector进入与Skill相同的公共Artifact/API/下载/前端卡片标准，覆盖ordinary、approval、remote、60秒post-ready历史补投、补投先于GC、目标盘容量门禁、按safe Call隔离的闭合前端/history状态，以及Artifact接管后durable源`artifact_owned → deleted`的真实生命周期。
- `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-implementation-plan.md`：上述设计的待执行实施计划；按基线红测、Storage/projector、ordinary/remote接入、post-ready补投与GC、闭合事件/Task Summary/frontend状态、完整回归和真实OCR下载smoke分Phase实施并设置3个代码checkpoint。
...
| `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-design.md` | 98%通过信心门；尚未实施 | 复用现有MCP durable-result promotion与Skill公共Artifact交付链，实现每个成功Call一个原始JSON Artifact、统一四路径projector、post-ready补投/GC竞态安全、多Call/history闭合状态提醒、无网络重放补偿和真实OCR下载验收。 |
| `superpowers/specs/2026-08-19-mcp-tool-result-shared-artifact-standard-implementation-plan.md` | 待实施 | 依次完成projector authority、四路径接入、60秒补投先于GC、Task Summary/history状态、自动追踪矩阵与真实OCR smoke；每个checkpoint只暂存计划内文件。 |
