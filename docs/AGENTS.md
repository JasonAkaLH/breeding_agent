...
| PRD | 状态 | 后续动作 |
|---|---|---|
| `prd/backend/unified-agent-loop/README.md`（来源：统一Agent Loop架构与PRD decomposition design） | Phase 0、Phase 1 `proof_complete`；真实隔离PG与真实Rust Sidecar证据闭合 | 进入P2-A Invocation Kernel行为保持提取；保持Phase 6前当前DAG运行时不变。 |
