...
- `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p3-implementation-plan.md`：基于P2 handoff与`5d7a360` HEAD启动的P3 Integrations边界计划；只合并Agent Skills纯解析与MCP附件metadata exact helpers，并让Integrations storage consumers采用P1窄ports；Gateway/Coordinator/Parser/secure authority控制流、90个C901与deferred behavior不动。
...
| `superpowers/specs/2026-08-25-project-wide-progressive-architecture-cleanup-p3-implementation-plan.md` | P3实施`active`；10组exact duplicate、90个C901、storage边界及平台条件已审计 | 按A～E只处理两类pure helper复用和窄port adoption，随后Backend全量；不进入P4。 |
