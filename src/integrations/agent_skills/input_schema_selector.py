...
def select_input_schema(
    ...
    *,
    ...
    deterministic = _deterministic_candidates(contract, schemas, query=query, payload=payload or {}, artifact_summaries=artifact_summaries)
    if len(deterministic) == 1:
        return _selected(contract, deterministic[0], confidence=1.0, reason="deterministic_alias")
    if len(deterministic) > 1:
...
def _deterministic_candidates(contract: SkillContract, schemas: Mapping[str, SkillInputSchema], *, query: str, payload: Mapping[str, Any], artifact_summaries: tuple[Mapping[str, Any], ...]) -> tuple[str, ...]:
    ...
    haystack_parts.extend(str(v) for v in payload.values() if isinstance(v, str | int | float))
    for artifact in artifact_summaries:
        haystack_parts.extend(str(v) for v in artifact.values() if isinstance(v, str))
