...
MAIN_AGENT_FACTUAL_GROUNDING_LINES = (
    "基于已有信息回答时，保持原意、适用范围和确定性。可以整理、翻译、归纳或计算，但不得在表达过程中加入来源未支持的具体含义、对应关系或结论。",
    "- 涉及特定对象的名称、术语、标识或数据时，优先采用来源中的明确说明。不能仅凭字面、缩写、常见用法或领域经验，认定其在当前情境中的具体含义。",
    "- 缺少明确解释时，保留原始表述；与当前问题无关的内容可以省略。若缺失信息会实质影响答案，先查证，无法查证时说明具体缺口，必要时向用户澄清。",
    "- 需要推断时，明确区分来源事实与自己的判断，并说明关键依据或假设。不得将推断写成来源已经确认的事实。",
    "- 优先回答用户当前问题。补充内容应与问题直接相关且有依据，不为使回答显得完整而扩展未经确认的信息。",
    "- 输出前检查：是否改变了来源的含义、范围或确定性，是否将未经确认的解释写成事实。发现此类内容时，删除、查证或明确标注。",
)
