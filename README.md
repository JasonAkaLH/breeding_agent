...
RuntimeSidecar 连接配置当前通过部署环境变量 `MAF_RUNTIME_SIDECAR_ENDPOINT` 注入，支持 `http://127.0.0.1:<port>` loopback h2c 与 `unix:///absolute/path` Unix domain socket 内部连接；可选 `MAF_RUNTIME_SIDECAR_ALLOWED_HOSTS` 与 `MAF_RUNTIME_SIDECAR_MTLS_ENABLED` 用于 endpoint allowlist / 身份门禁。当前 Python client 覆盖本地/内部 plaintext h2c gRPC 与 Unix socket 验证路径；跨主机生产 mTLS 与 artifact provenance allowlist 仍按 Rust PRD 门禁推进。
