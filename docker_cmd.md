...
按顺序逐行执行以下命令即可拉取并启动 backend `0.1.6` 与 frontend `0.1.6` 版本镜像。
...
docker run --rm --entrypoint python \
  ...
  registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-backend:0.1.6 \
...
docker image inspect registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-backend:0.1.6 --format 'tag_image_id={{.Id}}'
```

期望结果：两行 ID 一致。如果不一致，说明当前 `breeding-agent-backend` 容器不是用刚检查过的 `0.1.6` 镜像创建的，删除并按下面的标准 `docker run` 命令重建 backend 容器。
...
```bash
docker pull registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-backend:0.1.6
```
