...
按顺序逐行执行以下命令即可拉取并启动 backend `0.1.12` 与 frontend `0.1.12` 版本镜像。
...
```bash
docker pull registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-backend:0.1.12
```

```bash
docker pull registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-frontend:0.1.12
```
...
```bash
docker run -d --name breeding-agent-frontend --network breeding-agent-net -p 51999:80 registry.cn-hangzhou.aliyuncs.com/biobin/breeding-agent-frontend:0.1.12
```
