...
""", encoding="utf-8")
    ...
    async def test_v2_raw_answer_dto_extracts_canonical_values_and_keeps_chat_raw(self) -> None:
        ...
        async def slot_generator(prompt: str, **kwargs) -> str:
            if '"mode": "interrupt_turn_understanding"' in prompt:
                self.assertEqual(kwargs.get("metadata", {}).get("main_agent_reasoning_effort"), "max")
    ...
    async def test_v2_interrupt_clarification_keeps_interrupt_open_and_does_not_resume(self) -> None:
        ...
        answer = await self.client.post(
            ...
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-clarify-format",
                "answer": {"text": "这个列数应该填什么格式？"},
            },
        )
        self.assertEqual(answer.status_code, 202, answer.text)
        payload = answer.json()
        self.assertEqual(payload["action"], "clarification_answer")
    ...
    async def test_v2_interrupt_invalid_understanding_response_keeps_interrupt_open(self) -> None:
        ...
        answer = await self.client.post(
            ...
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-invalid-understanding",
                "answer": {"text": "12列？"},
            },
        )
        self.assertEqual(answer.status_code, 202, answer.text)
    ...
    async def test_v2_interrupt_invalid_resume_verifier_keeps_interrupt_open(self) -> None:
        ...
        answer = await self.client.post(
            ...
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-invalid-verifier",
                "answer": {"text": "12列"},
            },
        )
        self.assertEqual(answer.status_code, 202, answer.text)
    ...
    async def test_v2_field_shaped_answer_payload_is_rejected(self) -> None:
        ...
        field_shaped_answer = await self.client.post(
            "/api/v1/tasks/interrupts/answer",
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-reject-field-shaped-answer",
                "answer": {"ncols": 12},
            },
        )
        self.assertEqual(field_shaped_answer.status_code, 400, field_shaped_answer.text)

        internal_field_answer = await self.client.post(
            "/api/v1/tasks/interrupts/answer",
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-reject-internal-answer",
                "answer": {"text": "12列", "resume_interrupted_node_id": "node-x"},
            },
        )
        self.assertEqual(internal_field_answer.status_code, 400, internal_field_answer.text)
    ...
    async def test_answered_v2_interrupt_replays_same_turn_but_rejects_new_key(self) -> None:
        ...
        stale_answer = await self.client.post(
            "/api/v1/tasks/interrupts/answer",
            json={
                "task_id": task_id,
                "interrupt_id": interrupt["interrupt_id"],
                "client_request_id": "req-v2-stale-new",
                "answer": {"text": "13列"},
            },
        )
        self.assertEqual(stale_answer.status_code, 400, stale_answer.text)
        self.assertEqual(
            len(await self.runtime.storage.list_messages_for_conversation("conv-v2-stale-interrupt")),
            len(messages_before),
        )
