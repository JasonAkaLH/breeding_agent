...
class SlotStateMachineTest(unittest.TestCase):
    ...
    def test_backend_resolves_bare_scalar_with_single_planner_target_slot_hint(self) -> None:
        tmp, schema = _field_design_schema()
        self.addCleanup(tmp.cleanup)
        collection = replace(
            _collection(schema),
        ...
        without_hint = build_backend_slot_extraction(
            collection,
            schema,
        ...
        with_hint = build_backend_slot_extraction(
            collection,
            schema,
            current_user_answer="10",
