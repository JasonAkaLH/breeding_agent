...
class MainAgentWorkflowAndExecutorTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_soft_skill_binding_answer_uses_public_profile_without_running_scripts_or_raw_body(self) -> None:
        ...
        self.assertIn("Skill 文档事实约束", seen_prompts[1])
        self.assertIn("当前 Skill 文档未说明", seen_prompts[1])
        combined_prompts = "\n".join(seen_prompts)
    ...
    async def test_soft_skill_binding_llm_calls_inherit_frontend_model_and_thinking_options(self) -> None:
        ...
        async def streamer(
            ...
            **_kwargs: object,
            ...
            if stage == "soft_skill_decision":
                yield {"reasoning": "先判断是否执行。", "answer": None}
        ...
        self.assertEqual(
            [event.payload["delta"] for event in transient_events if event.event_type == "soft_skill.reasoning_delta"],
            ["先判断是否执行。"],
        )
        final_event = next(event for event in result.events if event.event_type == "main_agent.output_final")
...
""",
        ...
        self.assertIn("Skill 文档事实约束", prompts[0])
        self.assertIn("不得用通用领域常识替换文档口径", prompts[0])
...
""",
    ...
    async def test_auto_run_script_runtime_missing_input_uses_runtime_answer_table_question(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            skill_dir = Path(tmpdir) / "scripted"
            scripts_dir = skill_dir / "scripts"
            scripts_dir.mkdir(parents=True)
            (scripts_dir / "answer.py").write_text(
                textwrap.dedent(
                    ...
                    print(json.dumps({
                        ...
                        "missing": ["choice"],
                        "answer": "识别到多个候选项，请根据下表补充 choice。",
                        "columns": ["编号", "名称"],
...
""",
            ...
            async def slot_generator(prompt: str) -> str:
                question_prompts.append(prompt)
                return '{"question": "通用追问不应出现。", "ask_fields": ["choice"]}'
            ...
            result = await MainAgentExecutor(
                ...
                CapabilityExecutionRequest(
                    ...
                    task_id="task-1",
                    node_id="node-1",
                    input_payload={"user_message": "请选择候选"},
                )
            )

        output = result.output_payload["script_results"][0]["output"]
        self.assertEqual(output["missing"], ["choice"])
        self.assertEqual(prompts, [])
        self.assertEqual(question_prompts, [])
        self.assertIsNotNone(result.interrupt)
        assert result.interrupt is not None
        self.assertIn("识别到多个候选项", result.interrupt.question)
        self.assertIn("| 编号 | 名称 |", result.interrupt.question)
        self.assertIn("| 1 | 候选A |", result.interrupt.question)
        slot_collection = result.interrupt.required_fields[SLOT_COLLECTION_FIELD]
        self.assertEqual(slot_collection["missing"], ["choice"])
