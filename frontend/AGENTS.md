...
- `src/api/taskEvents.ts`, `src/domain/taskEvents.ts`: authenticated task-event transport and deterministic reducer；additive消费closed Agent frontend events，按interrupt/node跟踪multi-waiting，仅渲染transient Agent reasoning；CP7 terminal events和MCP result-Artifact projections使用closed payload、canonical per-Call folding、冲突检测和显式resync。
